...
- Phase 1 后的目标架构已修订为 `ResolvedGame / GameSimulation / GameCommandExecutor / GameQueries`；当前仍是包含 Modules、Sandbox、Demo 与空 E2E package 的 Phase 1 过渡布局，Phase 2 Task 4 才迁移为只保留 E2E/PoC Story 的目标布局；
...
从 [Goal 入口](docs/engineering/GOAL.md) 判断 live state：先完成独立素材轨道并提交可为空的 approved pack 交接，再单独执行 Phase 0 物化计划；只有本地离线凭据通过后，才能创建从 Phase 2 开始的长工程 Goal。该 Goal 交付可复现的本地 Artifact，但不会生成或批准素材、宣告主观试玩通过、创建 GitHub Actions 或执行远端发布。完整自动化验收之后再启动最终人工审查；远端分发另行设计和批准。

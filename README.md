...
- Phase 2 已采用 `ResolvedGame / GameSimulation / GameCommandExecutor / GameQueries` 架构；workspace 只保留独立的 E2E 与 PoC Story，E2E 暂时承接确定性 counter walking Story，PoC 保持无副作用的空 package 边界；
- Phase 2–6 已依据实际 Phase 1 实现完成修订；独立素材交接与 Phase 0 本机依赖、浏览器物化均已完成，长工程 Goal 正按 live checkpoint 从 Phase 2 向后推进；
...
从 [Goal 入口](docs/engineering/GOAL.md) 根据 live repository state 恢复并继续 Phase 2–6 的本地工程 Goal。该 Goal 交付可复现的本地 Artifact，但不会生成或批准素材、宣告主观试玩通过、创建 GitHub Actions 或执行远端发布。完整自动化验收之后再启动最终人工审查；远端分发另行设计和批准。

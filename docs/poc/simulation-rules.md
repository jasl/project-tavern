...
模拟核心拥有唯一的 `GameState`。UI 不直接修改状态，只提交命令并渲染结果。

概念模型：

```text
GameState
├── meta        schema、内容 revision、runId、runStatus、命令序号
├── clock       当前日、时段、生活方针、剩余 AP
├── player      体力、上限、属性段位
├── heroine     体力、心情、好感、默契、关系阶段
├── tavern      现金、人气、库存批次、设施、伙计、今日计划
├── auras       PoC 允许的少数 Aura 实例
├── story       机会状态、路线事实、事件结果
├── rng         算法名与可序列化内部状态
└── audit       最近命令、facts 与账本摘要
```

静态的原料、菜谱、需求、设施和事件定义属于 `GameDefinitions`，不写入每份存档；存档只记录其稳定 ID 与内容 revision。
...
晚上有两个独立事务：先执行 `ResolveService`（或确认计划停业），再执行 `AdvancePhase`。前者只结算营业并设置 `eveningResolved=true`，绝不改变时段；后者要求该标记存在，完成腐败、日终状态、恢复和次日推进。这样时段永远只有一个所有者。
...
`ResolveService` 必须按以下固定顺序原子执行：

1. 验证日期、计划、营业方式、AP、体力、现金和原料；
2. 扣除营业 AP、体力、工资和杂费；
3. 按 FIFO 消耗计划份数所需原料；
4. 读取并记录当晚实际客流；
5. 计算菜单有效订单；
6. 应用接待、备菜、设施和 Aura 容量；
7. 计算销量、收入、未售成品与缺失订单；
8. 更新现金、人气、默契、心情与事件事实；
9. 消耗“下一次营业”Aura；
10. 生成完整 ledger facts，并设置 `eveningResolved=true`；
11. 提交事务并更新滚动自动存档，但保持当前时段为晚上。
...
计划停业也通过 `ResolveService` 生成一个零收入的合法晚间结算，再由同一个 `AdvancePhase` 完成日终。不存在第二条偷偷跨日的路径。
...
存档只发生在命令或结算事务完成后。至少保存：

- `schemaVersion`；
- `gameVersion` 与 `contentRevision`；
- 完整 `GameState`；
- PRNG 算法和状态；
- 已提交事件结果；
- 最近命令序号和校验摘要。

加载顺序：解析 JSON → 校验外壳 → 逐版本迁移纯数据 → 校验当前 schema → 校验 `contentRevision` 与内容引用 → 恢复状态。比当前程序更新的存档必须拒绝加载；内容 revision 不一致也默认拒绝，除非存在显式内容迁移。

原型期允许宣布旧存档失效，但仍要保留至少一个 v1 fixture，证明迁移骨架和 round-trip 工作。
...
Fact 包含稳定原因码与数值 before/after。UI 文案和测试都消费 facts，不通过重新猜测状态差量来解释结果。`GameState` 快照是唯一恢复来源；facts 只负责本次解释和最多 200 条的有界诊断记录，不用于事件重放。只有按日/按周汇总、具有游戏语义的营业账本持久化。

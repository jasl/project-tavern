...
- [`superpowers/specs/2026-07-10-seven-day-tavern-poc-design.md`](superpowers/specs/2026-07-10-seven-day-tavern-poc-design.md)：形成当前合同前的设计评审摘要；保留推理背景，但不作为实现冲突时的权威来源。
...
- [`architecture/web-poc-architecture.md`](architecture/web-poc-architecture.md)：早期一次性灰盒架构，保留历史推理；与新 Harness 规格冲突时不再作为权威。
...
- **已形成基线、待书面复核**：首个可玩物是七日网页 Story；Harness 代码按生产级维护；模拟核心与 UI 分离；无后端；固定种子；IndexedDB 本地存档；Story 可实现受控强类型规则接口。
- **本轮可调**：所有经济数值、行动/体力强度、需求与委托效率。
- **等待试玩**：行动点与体力是否重复、经营步骤是否疲劳、预测信息量、三种营业模式是否都有用途。
- **明确延后**：Unity、正式美术、完整关系阶段、完整设施树、多周目、Mod、运行时 LLM、成人版本。

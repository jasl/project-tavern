...
pnpm --filter @project-tavern/story-poc exec vitest run src/application/create-poc-presentation-runtime.test.ts src/application/poc-application-root.test.tsx
pnpm --filter @project-tavern/story-e2e exec vitest run src/application/create-e2e-presentation-runtime.test.ts src/application/e2e-application-root.test.tsx
...
Retain the existing `walking-skeleton.spec.ts` first E2E Semantic command as its bounded `@smoke` owner. Split the PoC first ordinary action into one bounded test whose title includes `@smoke`; the complete E2E primary-flow evidence in this task must not add a duplicate smoke tag. Subsequent Save/reload/workflow coverage stays in the surrounding `@phase5c @primary-flow` suite without that tag. Together with Task 6's two smoke cases, `rg -n "@smoke" engine/packages/web/e2e` must find exactly four executable test titles.
...
The same live browser RED also proved an accepted Phase 5A focus edge: completing the E2E
Narrative legally disables its original `start` opener, so the existing cleanup attempted to focus
a connected but disabled button and left focus on `body`. The unique generic repair is also owned
here: if the exact opener is no longer focusable, restore focus to the first still-enabled logical

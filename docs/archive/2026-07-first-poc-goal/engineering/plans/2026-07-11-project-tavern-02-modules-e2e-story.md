...
- Create: game/stories/e2e/src/gameplay/contracts/simulation.ts
- Create: game/stories/e2e/src/gameplay/contracts/index.ts
- Create: game/stories/e2e/src/gameplay/contracts/contracts.test.ts
...
- Test: game/stories/e2e/src/gameplay/contracts/contracts.test.ts
...
- Produces: E2eGameStateV1、E2eGameCommandV1、closed replayable E2eDebugCommandV1/E2eDebugValidationErrorV1、E2eGameplayFactV1、E2eRejectionReasonV1、E2eGameplayFaultV1、E2eSimulationProgramInputV1、final invariant E2eGameSimulationTypesV1 witness、three stateful owners counter/flow/run、one provider-injected stateless choiceDeltaResolver module factory and exact read ports。
...
```bash
pnpm --filter @project-tavern/story-e2e exec vitest run src/gameplay/contracts src/gameplay/modules src/gameplay/resolvers
```
...
Facts、rejections 和 internal gameplay faults 同样是 exact closed unions：
...
`gameplay/contracts/simulation.ts` 在本任务定义唯一最终 invariant type spine；Task 8 只实现 factory/projector，不得替换 Module witness：
...
状态槽精确为 simulation.counter、simulation.flow 和 simulation.run。四个 descriptor 的 `contractRevision` 都精确为 1；ID/dependency/slot 和最终 tuple authored order 精确为：`e2e.counter`/`[]`/`[simulation.counter]`，`e2e.flow`/`[e2e.counter]`/`[simulation.flow]`，`e2e.run`/`[]`/`[simulation.run]`，`e2e.choice-delta-resolver`/`[]`/`[]`。Flow 只取得 CounterReadPortV1；Run 不直接依赖 Flow，由 executor 显式传入 terminal proposal input。

choiceDeltaResolver 是 stateless binding factory，stateSlots 为空，capabilities 只有 `resolveChoiceDelta(choice): PositiveSafeInteger`。Production 不导出内置 default binding/switch；Task 7 test 注入 test-local left→1/right→2 provider，Task 8 始终注入 `program.rules.resolveChoiceDelta`，Task 9 的 default/Hotfix Program provider 是 production 唯一 branch switch。capability 的返回合同不能把官方 Hotfix 锁死在 literal `1 | 2`。

Owner operation 和 normal transition 冻结如下：Counter 只接受 `counter.add { amount } | counter.set { value }`，proposal payload 只有 `{ value }`，safe-integer overflow 返回 `counter.value_out_of_range`；Flow 只接受 `flow.start | flow.choose { choice } | flow.continue | flow.set_blocked { blocked }`，proposal payload 是完整 next Flow slice；Run 只接受 `run.complete { terminal: { flowStatus, nodeId, counterValue, terminalThreshold } }`，proposal payload 只有 `{ status: "complete" }`。正常 Flow 路径精确为 `idle/null/intro → choosing/null/choice → blocked/<choice>/rejoin → resolved/<choice>/done`，choose proposal 的 Facts 顺序为 `flow.branch_selected` 后 `flow.blocked { blocked: true }`。start/choose/continue 的无效 current state 分别返回 `flow.not_idle`、`flow.not_choosing`、`flow.not_blocked`。`flow.set_blocked` 只在 choosing↔blocked 间切换 status，保留 branch/nodeId 并产生一个 `flow.blocked` Fact；无效 current/target 组合返回 `flow.block_state_conflict`。`left`/`right` NodeId 在 v1 保留为稳定预留 ID，但正常闭集不进入它们。Run 只有在 current status 为 active、显式 terminal DTO 为 resolved/done 且 `counterValue >= terminalThreshold` 时才能 proposal complete；否则分别返回 `run.not_active`/`run.not_terminal`，且 Run descriptor 不声明 Flow dependency。Task 8 的 terminal guard、显式 reject/fault command 和 contract-fault normalization 分别使用 `game.run_complete`、`test.rejected`、`e2e.test.fault`、`e2e.owner.contract_invalid` 和 `e2e.runtime.unexpected`，不得增加临时 string code。

所有 contracts/owner operation/proposal Schema 都拒绝 array、accessor、symbol、额外字段、非 plain object 和越界 safe integer；`e2e.counter.roll.maximum` 还必须不大于 `0x1_0000_0000`。Owner-specific proposal Schema 只接受本 owner 的 Fact variants，不能用完整 Fact union 让一个 owner 伪造另一个 owner 的 Fact。Counter/Flow/Run read port 和 Flow dependency object 都是新建的 exact frozen projections，不返回 State slice 或 Snapshot。Resolver capability 解析 input、委托 factory 注入的唯一 provider、再把返回值解析为 PositiveSafeInteger，不能保留第二份 switch。

当前根 `src/contracts.ts` 的 migrated counter skeleton ABI 仍被 Task 6 Story/Application 消费，本任务保持该文件和 `tsconfig.json` byte-identical，不把新 Command union 混入旧 executor。新的 canonical contracts 只从 `src/gameplay/contracts/index.ts` 导出；与 legacy 同名的 `E2eDebugValidationErrorV1` 只从该 canonical 路径消费。Task 9 切换完整 Story closure 前不得让旧 executor 接受新 Flow/Run Command。当前 `src/**/*.ts` 已覆盖新 Gameplay 文件，因此本任务不修改 tsconfig。
...
pnpm typecheck
pnpm verify:phase2:checkpoint
git diff --exit-code -- game/stories/e2e/src/contracts.ts game/stories/e2e/tsconfig.json
if rg -n 'e2e\.' engine/packages/base/src engine/packages/ui/src engine/packages/web/src; then exit 1; fi
git diff --check
```
...
```bash
git add -- game/stories/e2e/src/gameplay
git diff --cached --check
...
- Consumes: Task 7 bindings、E2eSimulationProgramInputV1、strict Game/Debug schemas、transaction RNG and Task 1 GameSimulation/GameDebugCommandExecutor contracts。
...
it("executes replayable debug commands through owner proposals", () => {
  ...
  expect(attempt.result).toMatchObject({
    kind: "committed",
    snapshot: { state: { simulation: { counter: { value: 5 } } } },
  });
});
...
`createE2eGameSimulationV1(program)` 精确接收 Task 7 的 `DeepReadonly<E2eSimulationProgramInputV1>`，并且只通过 `program.rules.resolveChoiceDelta` 创建 resolver binding。`canComplete`、`e2e.run.complete` queue-front guard 和 Run terminal DTO 共享同一公式：run active、flow resolved/done 且 `counter.value >= program.values.terminalThreshold`；不足时 normal executor 返回 `run.not_terminal`，不能让 threshold 成为未使用的 digest-only value。

本任务必须让现有同名 `createE2eInitialSnapshotV1`/`createE2eSessionV1` 通过 canonical overload 或同时覆盖 legacy+canonical 的泛型实现接收 `E2eGameSimulationTypesV1`，并原样保留 legacy overload 的签名/行为和 Task 6 profile/application closure。Parallel 新名字 helper 只能作为内部实现，不能替代同名 public helper；这样 Task 9 切换 Story 后，无需修改不在 Files 中的 `walking-skeleton.test.ts` 或再次修改 `session.ts`。不得为此修改 legacy `profile.ts`、root `contracts.ts` 或 Task 6 application。
...
**Accepted-owner repair before the Task 9 RED:** Task 2 accepted the StageScene/Interaction
contracts before the Contract Catalog's frozen `LocalizedTextCatalogV1` /
`TextCatalogSetV1` ABI and resolved SceneGraph TextId join were implemented. Per the execution
protocol's unique-answer earlier-owner rule, repair that Base owner first; this is not a new Task,
does not reorder the phase, and must be committed independently before the Story RED.
...
- Modify: engine/packages/base/public-exports.v1.json
- Modify: engine/packages/web/src/loader/loader.test.tsx
- Modify: game/stories/e2e/src/presentation.ts

Repair TDD and contract:

1. Add focused contract tests for exact plain-data catalog parsing, canonical BCP 47 Locale IDs,
   unique locales/TextIds, a default catalog with `fallbackLocale=null`, existing acyclic
   non-default fallback chains that terminate at the default, and deep-frozen parsed output. Add
   resolver tests proving that every SceneGraph TextId category, including content flag and preset
   name/description IDs, is joined against the active post-materialization TextCatalog and that a
   missing reference returns `story.presentation_invalid` with
   `presentation.catalog.missing_reference` in bounded details. Add a Hotfix resolver test proving
   a `text` slot accepts one complete canonical `TextCatalogSetV1` object rather than only a string.
2. Run
   `pnpm --filter @sillymaker/base exec vitest run src/contracts/presentation.test.ts src/authoring/hotfix-resolver.test.ts src/authoring/story-resolver.test.ts`
   and confirm the focused assertions FAIL because Locale/TextCatalog contracts and the TextId join
   do not exist. Syntax, environment, or unrelated failures do not count as this RED.
3. Export `LocaleId`, `LocalizedTextCatalogV1`, `TextCatalogSetV1`, `parseLocaleId`, and
   `parseTextCatalogSetV1` from Base's named public barrels. The parser preserves authored order,
   returns deeply frozen plain data, and does not invent a general localization service. Validate
   both the source catalog and the materialized Presentation's own `textCatalogs`; use the latter as
   the active post-Hotfix catalog for the complete SceneGraph TextId join. The materialized
   Presentation must therefore carry the complete resolved set that also enters
   `presentationDigest`. A `text` replacement is canonical serializable data, not necessarily a
   string; Hotfix resolution admits the complete object and the Story resolver applies the catalog
   parser afterward. Do not add slot-schema fields absent from the frozen v1 PatchSlot ABI.
4. Migrate only the accepted synthetic counter, the Web Loader's exact synthetic Presentation
   expectation, and the current E2E predecessor catalog needed to keep the owner-to-current gate
   green; the full E2E Story replaces the predecessor later in this Task.
5. Run:
   ...
   pnpm verify:public-exports
   pnpm verify:stories
   pnpm --filter @sillymaker/web exec vitest run src/loader/loader.test.tsx
   pnpm verify:boundaries
   pnpm typecheck
   pnpm verify:phase2:checkpoint
   git diff --check
   ...
   Expected: all commands exit 0; source and active catalogs are frozen and validated; TextId
   failures remain presentation failures; no Story, UI, Web, fixture, golden, or lockfile bytes
   change beyond the listed predecessor consumers.

6. Commit the accepted-owner repair exactly:

   ```bash
   git add -- engine/packages/base/src/contracts/presentation.ts engine/packages/base/src/contracts/presentation.test.ts engine/packages/base/src/contracts/index.ts engine/packages/base/src/authoring/hotfix-resolver.ts engine/packages/base/src/authoring/hotfix-resolver.test.ts engine/packages/base/src/authoring/story-resolver.ts engine/packages/base/src/authoring/story-resolver.test.ts engine/packages/base/src/testkit/synthetic-counter.ts engine/packages/base/src/index.ts engine/packages/base/type-tests/phase1-consumer.test-d.ts engine/packages/base/type-tests/public-exports.test-d.ts engine/packages/base/public-exports.v1.json engine/packages/web/src/loader/loader.test.tsx game/stories/e2e/src/presentation.ts
   git diff --cached --check
   git commit -m "fix(base): validate resolved text catalogs"
   ```
...
- Create: game/stories/e2e/src/simulation/choice-delta-hotfix.ts
...
it.each([
  ...
  (_kind, presentation) => {
    expect(resolveE2eStoryWithPresentationFixtureV1(presentation)).toMatchObject({
      kind: "failed",
      failure: {
...
Executable provider 与 Hotfix implementation 分别放在无 generated-digest 反向依赖的
`choice-delta-provider.ts`/`choice-delta-hotfix.ts` source roots。受控 writer 对各自精确的
workspace-relative POSIX path 与文件 SHA-256 运行对应的 domain-separated canonical digest，
生成 `source-digests.generated.ts`；read-only test 从 live bytes 独立重算并拒绝 stale 常量。
不得用 path/symbol/revision metadata、`Function#toString()`、mtime 或绝对路径代替真实 source
...
`content-maturity-policy.ts` 还从同一组已解析值导出 `e2eAlphaFlagV1`、`e2eBetaFlagV1`、`e2eBothFlagsV1` 和 `e2eStreamSafeContentPresetIdV1`；policy descriptor 直接复用这些常量，不能再写第二份裸数字。它们是后续 Phase 5B 中性 Story fixture 的唯一 flag/preset 常量来源，不进入 Base，也不从 PoC 导入。`createE2ePresentationWithMissingFlagTextV1`/`...PresetTextV1` 与 `readResolvedE2eTextV1` 都是 `story-contract.test.ts` 的 test-local helpers，前两者保留合法 SceneGraph 但从 active `TextCatalogSetV1` 删除恰好一个引用；存在性由拥有两侧数据的 Base Story resolver 验证，helper 只构造 fixture 和检查公开 resolution result，不能伪造 inner error code。
...
既有 `property.test.ts` 的 legacy counter helper 不得在 1,000 次 property run 的每次
`run()` 中重新执行完整 Story resolution；Task 9 的真实 TextCatalog/SceneGraph startup
validation 使该循环成为 2,000 次无关的 startup benchmark。测试模块只 resolve 一次同一
immutable GameSimulation，每个 seed/count case 仍创建两个全新 Session，并原样保留
1,000 runs、seed/count 范围和确定性断言；不得通过降低 runs 或放宽 timeout 规避。
...
pnpm --filter @project-tavern/story-e2e exec vitest run src/simulation/source-digests.test.ts
pnpm verify:stories
pnpm verify:assets
pnpm verify:boundaries
pnpm typecheck
...
it("publishes game, narrative, and actions atomically from exactly one Queries instance", async () => {
  ...
  expect(committed.narrative).toBeNull();
...
- 第一次 publication 以及每次 authoritative token 替换时，各从当前 committed Gameplay State 恰好创建一个 Queries；同一个 Queries 同时投影 `game`、`narrative` 和 `actions`，随后原子冻结/发布，不能先后读取两次 State 或建立第二套 action gate；
- status-only publication 只替换 `status`/publication envelope，并严格复用前一 publication 的 `game`、`narrative` 与 `actions` 引用；`observe()` 是只读缓存读取，`availableActions()` 必须返回当前 `observe().actions` 的同一引用，二者都不能新建 Queries；
...
`createE2eSemanticGamePortV1` 必须把 `gameSimulation.createQueries`、`gameSimulation.projectGameView`、恒返回 `null` 的 `projectNarrativeView`、`createE2eSemanticActionCatalogV1` 和 `previewE2eSemanticInvocationV1` 直接交给 Base factory；E2E Story 的 `narrativeProgram=null`，所以不伪造 Narrative DTO。不得新增 `projectE2eSemanticGameViewV1`、另一个 State reader 或另一个 Gameplay view model。每个 authoritative token 只创建一个 Queries，并由它同时生成 GameView、null NarrativeView 与 action catalog。`queryWitness` 只存在于 test fixture，用来证明 publication 的 game/narrative/actions 同源，不进入 production descriptor。
...
- atomic game/narrative/actions publication、status-only reference reuse 和 waitForIdle revision behavior without sleep；

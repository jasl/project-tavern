...
状态：Phase 6 release calibration 已冻结；完整 1..1000 gate 通过
...
- Root/Story `pnpm verify:balance` 是唯一完整 release semantic gate。其 CLI 直接运行 seeds 1–1000、全部六策略、D4 压力、设施 counterfactual 和本节所有总体阈值，最终只输出一个包含 `deficit` 与精确 metrics/counterfactual evaluation 的 canonical report；它不通过 Vitest、`test:story` 或 smoke alias 间接运行。普通 Phase 4B/5、unit 与 full local verification 不得意外包含这条长 corpus。完整 evaluator 可显式接收 `--workers=1..64`，本机默认 16；worker count 只划分连续 seed ranges，不进入 canonical report/evidence。Remote strict 不是第三种 gate：它从 exact clean archive 直接执行这个 default CLI，并把相同 canonical stdout 交回本机 Story admission。

`pnpm --filter @project-tavern/story-poc calibrate:balance --iteration=N` 复用完整 corpus，但只读地枚举和评估 §14.4 的合法邻居，输出 canonical baseline/candidate/selection evidence；可验证辅助执行使用 `calibrate:balance:remote --iteration=N [--prior-after-sha256=sha256:<previous-accepted-after>] --host=... --remote-root=... --workers=... --attestation-out=...`，但 semantic evidence 与本机命令保持同一形状。`N = 0` 禁止 prior digest；`N > 0` 必须在 iteration 后紧接上一 accepted step 的 admitted after-evaluation digest。`N` 只能来自 accepted Phase 5C checkpoint 后 first-parent ancestry 中已验证且严格连续的 calibration-step commit 数，不来自聊天、shell 或本地文件。它显式表示已应用的变化数，调用时不插入独立 `--`，也不修改 balance、fixture、golden、Save 或任何计划文件。Final strict 使用无 package alias 的 direct controller grammar `node scripts/run-poc-balance-remote.mjs --strict --host=... --remote-root=Workspace --workers=1..64 --attestation-out=<absolute-path>`，拒绝 iteration/prior 参数。Strict remote result 与 execution attestation 分别使用 `project-tavern.poc-balance-remote-strict-result.v1` 和 `project-tavern.poc-balance-remote-strict-execution.v1`；本机从同一 frozen archive 重编码并 Story-full-admit report、独立导出零 deficit 后才接受 stdout/attestation。默认 strict `verify:balance` 对任何非零 `deficit` 都失败。
...
Phase 6 在 Phase 5C Acceptance 后完成了一个 `openingFee: 2 → 1` calibration step。seeds `1..1000` 的 canonical report 为 `deficit=0`，本节全部冻结阈值和 counterfactual 均通过；唯一 final balance-freeze commit 的 `Balance-Calibration-Report-SHA256` trailer 绑定该 report 的精确 bytes。

完整校准已在 Phase 5C Acceptance 之后、任何 Phase 6 Artifact implementation/build/release evidence 之前闭环；此前 Phase 5 development builds 只属于 UI/interaction evidence，不能充当 release evidence。Phase 6 entry 紧邻 accepted Phase 5C，因此从该 checkpoint 到 final（尚无 final 时到 `HEAD`）的每个 first-parent commit，不论 path，都必须且只能分类为连续的 `Balance-Calibration-Index: 1..N` step、显式 `Balance-Calibration-Repair: true` 的闭集 repair，或唯一 final；两个 repair subtype 互斥：Task 10 executable-owner repair 必须至少修改一个列出的非权威文档 executable/test/package path，calibration/finalization authority repair 的非空 diff 则只能修改 GOAL/Roadmap/local-engineering delivery-boundaries/balance-lab/Phase 4B plan/Phase 6 plan/本文且不改变任何 executable、balance/direct expectation、qualifier、fixture 或 lock byte。final 必须晚于最后 step/repair。Final 后允许普通 Phase 6 task commits，但以 `git diff "<commit>^1" "<commit>"` 检查时不得再触及 step/final protected paths。任何无分类/多分类 pre-final commit 使恢复无效。

Strict finalization 使用第二个 executable-owner repair，其 Files 必须恰好是
`game/stories/poc/src/testing/balance-calibration.ts`、`game/stories/poc/src/test/balance-1000-seeds.test.ts`、
`scripts/verify-poc-balance.mjs`、`scripts/verify-poc-balance.test.mjs`、
`scripts/run-poc-balance-remote.mjs`、`scripts/run-poc-balance-remote.test.mjs`，加 GOAL、Roadmap、local-engineering
delivery-boundaries、balance-lab、本 Phase 4B plan、Phase 6 plan 与本文七份 authority，共 13 paths。它只增加 additive
Story full-report admission、default-strict admission hardening、独立 remote strict contracts/orchestration 和
parent-report candidate injection proof；不得改变 calibration evaluator/runner/selector/codec outputs、balance/direct
expectations、provisional/fixture/manifest/lock bytes。它按 Phase 6 plan 的 named expected RED、focused gates、历史
calibration replay 和 exact staged-set 合同验收，提交固定为
`fix(story-poc): attest remote strict balance verification` 且只带 `Balance-Calibration-Repair: true`，不消耗 calibration
index，也不替代既有 Save provenance finalization repair。

所有 historical replay 与 dirty recovery 的本机 selector、writer、patch 和 admitted-candidate 重建都在 temporary detached clean sandbox 中运行，使用 live store 的 recovery-only offline frozen install。Final-parent/final full-corpus strict 是显式例外，只从对应 exact clean archive 在辅助执行器运行：
...
执行者先选择 Phase 0 materialized PATH（当前 checkpoint 为 `/opt/homebrew/bin`），live 与 subshell 的 Node/pnpm 断言通过且匹配 accepted materialization identity 后才可产证据。该 install 不访问 registry、不改变 live tree/lockfile。每个历史 step 都必须在其 parent sandbox 以当时已应用步数 `N` 重跑本机 `calibrate:balance`，或从该 parent exact archive 重建远端 canonical aggregate evidence 并执行下述本机 admission/selector/digest-chain 验收；两种方式都校验 canonical evidence SHA-256 与所有 field/before/after/deficit/index trailers，重新把候选应用到 `balance-v0.md`、`balance.ts` 和精确 direct literals，并要求重建的完整 `git diff --binary` 与历史 commit byte-for-byte 相同。只验 path/scalar 不足以恢复。每轮新候选仍只改一个既有数值并独立提交，step 不含 qualifier、golden 或 Save。
...
IP、路径、时间或调度顺序写入 semantic evidence/attestation。远端 semantic value 必须包含 current 的完整 1..1000
aggregate metrics/counterfactual evaluation、每个 canonical candidate 的同形 evaluation，以及 claimed selection；它不是
opaque score。本机不重复执行完整 corpus，而是用同一 Story codec 对 bytes 重新 canonicalize，严格验证 schema、
iteration/current values、aggregate evaluation、legal-neighbor identity/order/completeness 或首零前缀，并独立重算
deficit/selector。`N = 0` 的 before digest 来自 admitted current evaluation；每轮 after digest 来自 admitted selected
evaluation；`N > 0` 的 before digest 必须精确等于上一 accepted step 的 after digest。任何 framing、identity、admission、
prefix、selection、attestation 或 digest-chain mismatch 都停止，绝不应用候选。

同一 controller 的互斥 `--strict` mode 只在 clean final-parent/final exact archive 中直接运行 default
`scripts/verify-poc-balance.mjs --workers=<workers>`，不带 calibration/qualifier 参数，只接受 status 0 和唯一单行
`PoC balance report <canonical-json>\n`。远端 envelope 使用
`project-tavern.poc-balance-remote-strict-result.v1`；本机从同一 frozen archive 加载 Story codec 与
`admitPocBalanceFullReportV1`，要求 exact report keys、完整 evaluation range/count/identity/counterfactual、canonical
bytes 与 derived zero deficit 全部成立。独立 strict attestation 使用
`project-tavern.poc-balance-remote-strict-execution.v1`，精确字段为 `schemaRevision`、`contractId`、`sourceCommit`、
`sourceTree`、`sourceArchiveSha256`、`pnpmLockSha256`、`materializationDigest`、`packageClosureDigest`、`nodeVersion`、
`nodeArchiveSha256`、`pnpmVersion`、`pnpmArchiveSha256`、`platform`、`arch`、`availableParallelism`、`workerCount`、
`firstSeed`、`lastSeed`、`seedCount`、`evaluationSha256` 与 `reportSha256`。`seedCount` 固定 1000；
`reportSha256` 的字节域是完整 report stdout line，`evaluationSha256` 的字节域是 admitted evaluation canonical bytes。
Attestation 仍不得包含 host/path/time/elapsed/shard ordering，且始终写在仓库外。
...
若改变 evaluator/runner/selector/evidence codec 或其语义输入的 executable-owner `Balance-Calibration-Repair: true` 出现在 `N > 0`，必须从 Phase 5C sandbox overlay repaired evaluator，并从 `--iteration=0` 顺序重放全部旧 steps。只改变 remote controller 验收/编排的 controller-only orchestration repair 必须把 repaired controller 作为每个原 step parent exact archive 之外的 admission 层运行，保持原 archive commit/tree/SHA-256 与 trailers，不得把 controller repair overlay 进历史 archive。Exact-13 strict-finalization repair 使用同一路径在每个原 step parent archive 外运行 repaired calibration controller，必须让原 canonical calibration evidence、旧 calibration attestation bytes、trailers 与 full binary patch 全部 byte-identical；`balance-calibration.ts` 的 additive full-report admission 不得改变原 codec/admission/selector output。若是 authority-only calibration/finalization repair，则先证明该 repair 的 parent→commit patch 对 calibration executable/import closure 的投影为空，再在每个历史 step 的原 parent/exact archive 重放，不能把无关文档 overlay 后改变原 source-archive identity。所有情况都要求每轮 evidence/trailers/full binary patch 全等才能继续。任一差异产生 `balance_calibration_history_invalidated` 权威设计停点，禁止自动 rewrite、rollback 或重选历史；`N = 0` 可在 owner repair gates 通过后继续。Dirty step 必须从 clean-`HEAD` sandbox 重算/验收 selector/full patch；dirty final 只从 exact parent admitted report 重跑 provenance/writer/removals、candidate injected admission 与 exact closure/patch guards，不运行 full corpus。混合或范围外 dirty bytes 必须停止。
...
所有冻结阈值通过后，clean/no-final 首次收口先从 clean non-detached live `main@final_parent` 的 exact archive 运行一次 remote default strict；本机 controller 必须从同一 frozen archive Story-full-admit report、导出零 deficit，并把 parent canonical stdout 与 strict attestation 保存在仓库外。若 dirty candidate 恢复时这两份 evidence 丢失，只能从同一 exact parent 的 verified clean attached-`main` controller checkout 重建，不能从 dirty bytes 或 fixture 猜测。随后才投影完整 live Save fixture provenance；本次只允许 frozen `blocking.simulationDigest` 从 `sha256:015106ae1e2513fbf93c2fab692a119d580193e2267aa8b8ce55c0bf6ea3d1d7` 变为 `sha256:165ea3339e15f51bb92d1cf205e6cc4bc66e23fe5912262356405ad20484140c`，其余 blocking、diagnostic 与 capture tuple byte-identical。现有 complete-live-provenance focused test 必须先精确 RED，再在只更新该 frozen scalar 后 GREEN；不得放宽 generation guard 或修改 builder/writer/test。随后显式重新生成 Task 11 的 6 个 golden 与 Task 12 的 8 个 Save fixtures、执行精确 removals，并形成真正可 stage 的 candidate；final-parent sandbox 再独立重复同一 provenance 重绑定与 writers/removals，重做完整 diff、精确 file count、schema/provenance、负面单字段差异和两次按路径排序 SHA-256 审查。只移除 provisional report data、assertion、`--qualify-provisional` CLI branch、专属 tests 与本文 provisional-to-final 状态文字；包含 `game/stories/poc/src/testing/save-fixture-provenance.ts` 的完整 final `git diff --binary` 必须与 live candidate、historical final 或 pending dirty final 精确相同。

Dirty candidate 不运行 full corpus。它把实际 parent admitted evaluation 注入 candidate `runPocBalanceCliV1({ args: [] })`：真实 full-corpus evaluator/import 零调用，injected provider 恰好调用一次且只返回该 evaluation，candidate Story full-report admission 后 stdout 必须与 parent remote stdout byte-identical。Final diff 对 `balance.ts`、balance evaluator/calibration codec/counterfactual/strategy compiler+runner、两个 direct-expectation tests、manifests 与 lockfile 的投影必须为空；verify CLI/tests 只能包含计划精确列出的 provisional removals，并且完整 writer/removal binary patch 必须相同。唯一 final commit 使用 parent/candidate 相同 stdout 的 SHA-256，不再修改 `balance.ts` 数值或两个 direct-expectation tests，并带 `Balance-Calibration-Final: true`、`Balance-Calibration-Steps: N` 与 `Balance-Calibration-Report-SHA256: sha256:<digest>` trailers。

提交后从 exact final commit archive 进行两次独立、fresh remote default strict；两次 canonical stdout 与两份 canonical strict attestation 必须分别 byte-identical，stdout SHA-256 必须匹配 final report trailer。Strict attestation 不进入 final trailer：它绑定 final commit/tree/archive，把其 digest写回同一 commit 会形成 identity 循环；final 只记录 report digest。随后回到 clean live `main`、`HEAD = final`，再使 `verify:golden`、`verify:fixtures`、`pnpm verify` 与 materialization 全部通过；这些本机 gate 不重复 full corpus。完整 replayed step/repair chain 加 final commit 才构成允许进入 Phase 6 Artifact 工作的冻结 checkpoint。

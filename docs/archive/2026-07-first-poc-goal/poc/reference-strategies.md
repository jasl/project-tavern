...
若预留固定窗口后，普通动作无法按上述规则排入合法时段，则 driver 直接失败。Reference-seed command fixture 不是裸 invocation 数组，而是带身份的封闭 envelope；每个 entry 必须保存 `order`、提交前的 `day`、`phase`、`commandSequence` 和完整 `invocation`。`order` 从 0 连续递增，`commandSequence` 必须等于真实 Session 在该 invocation 前的权威序号；writer、reader 和 runner 都使用同一个严格 schema。表格本身不在测试运行时临时解释。
...
种子 `0x00000001` 到 `0x000003E8` 由同一个强类型 strategy compiler 复用第 4–9 节的同一日程、动作顺序、营业方式、菜单与份数，不直接重放只适用于 reference seed 的完整 literal fixture，不根据客流偏移改菜单，也不搜索最优解。compiler 在每次提交前只可读取公开 Semantic action/preview；唯一允许影响后续 invocation 选择的 Gameplay 状态分支是调查提交完成后的 D6 `fact.war_clue`：
...
D4 的 `buy(D4+D5+D6-clue)` 是在不知道骰点时做出的固定乐观采购承诺，不读取未来随机值；D6 的 `buy(current)` 再根据已提交结果、剩余库存和实际 D6 固定计划补足短缺。需求偏移只影响结算，不影响计划选择。compiler 必须通过一个穷举的 closed decision API 请求该分支；除 `war_clue ? cluePlan : noCluePlan` 外，任何按现金、体力、库存、客流、检定档位、拒绝原因或未来 RNG 改变 invocation 的代码都使测试失败。
...
这些产物由执行 Goal 的 agent 按固定技术 rubric 复核后进入 `src/test/fixtures/golden/`：严格 schema/closed ID、entry 顺序与 `day/phase/commandSequence` 连续性、同种子重放一致性、无 Debug/Cheat/私有状态读取、六晚账本守恒、终局结果和完整 SHA-256 清单。首次生成的未跟踪文件必须先 `git add -N`，再显示完整 diff 和按路径排序的 SHA-256；验证前后必须比较完整清单。后续规则或数值改动仍须显式更新 fixture、复核完整 diff 并说明原因，测试不能自动接受新输出。这里的 agent 技术复核不等于人工试玩、正式文案、美术或玩法品味批准；后四者全部在主线工程 Goal 通过后另建任务。

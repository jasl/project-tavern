...
- 最终 exact commit archive 的完整 default strict gate 在辅助执行器独立运行两次，产生相同 canonical report bytes 与 canonical strict attestation bytes；
...
所有历史 replay 与 dirty recovery 的本机 selector、writer、patch 或 admitted-candidate 判断都在同一种 clean-commit
sandbox 中执行，不读取 dirty root 的代码或依赖。Final-parent/final full-corpus strict 是显式例外，只从对应 exact
archive 在辅助执行器运行。Sandbox 从 live materialization 只读取得 store path，允许唯一的 recovery-only offline install：
...
在 final freeze 前，strict full-report admission/finalization orchestration 使用第二个 executable-owner repair。其 Files
恰好是六个 executable/test paths：`game/stories/poc/src/testing/balance-calibration.ts`、
`game/stories/poc/src/test/balance-1000-seeds.test.ts`、`scripts/verify-poc-balance.mjs`、
`scripts/verify-poc-balance.test.mjs`、`scripts/run-poc-balance-remote.mjs`、
`scripts/run-poc-balance-remote.test.mjs`，加上述七份 authority documents，共 13 paths；不得修改
balance/threshold/strategy/direct expectation、provisional report/qualifier、golden、Save、command/tooling fixture、manifest
或 lock bytes。Expected RED 必须证明 strict mode、独立 strict contracts与 full-report Story admission 尚缺失；GREEN
必须证明 strict/calibration grammar 互斥、remote default strict
只运行一次且不带 calibration 参数、Story full-report schema/identity/count/counterfactual/derived-deficit admission、strict
report 与 attestation canonicalization，以及 parent admitted evaluation 注入 candidate default path 时真实 full-corpus
evaluator/import 零调用、injected provider 恰好返回该 evaluation 且 stdout byte-identical。Focused gates 是两个 balance
Node test files、Story balance leaf、两次 smoke、commands、两个 direct-expectation tests、typecheck、targeted lint/Prettier、
`pnpm verify:docs`、`git diff --check` 与 lock/protected-byte equality。Repair controller 必须继续在每个历史 step parent
exact archive 外重放 calibration evidence/trailers/full binary patch；原 archive commit/tree/SHA-256 不变。Exact-stage
的 path count 与 sorted set 必须恰好等于这 13 paths，提交 `fix(story-poc): attest remote strict balance verification` 且只带
`Balance-Calibration-Repair: true`；提交后必须 clean 并重新通过 materialization。若其 diff 超过 additive full-report
admission、strict orchestration 与 default-strict admission hardening，或改变 evaluator/runner/selector/evidence codec
语义，则升级为 repaired-evaluator replay，不能使用本闭集。
...
step 数。Clean/no-final 首次收口先从 clean non-detached live `main` 的 exact final-parent archive 执行一次 remote
default strict，并在本机从同一 frozen archive Story-full-admit canonical report、导出零 deficit；随后才投影 live
provenance。只允许与 accepted balance steps 因果一致的冻结字段漂移，并要求其余 blocking、完整 diagnostic 与 capture tuple byte-identical。本 checkpoint
...
`git diff --binary` 与 live candidate 精确相同。Historical/pending-dirty final 使用同一 replay 比较。Dirty candidate
不得运行 full corpus：从 parent admitted report 取得 evaluation，注入 candidate
`runPocBalanceCliV1({ args: [] })` 的 deterministic provider；真实 full-corpus evaluator/import 必须零调用，provider
恰好返回该 evaluation，candidate Story full-report admission 后 canonical stdout 必须与 parent remote report
byte-identical。Final patch 对 balance/evaluator/codec/strategy protected paths 的投影必须为空，且
`scripts/verify-poc-balance.mjs`/tests 的变化只能是 exact provisional report/assertion/CLI branch/tests removals；该 exact
patch guard 与 injected-output equality 共同证明 default strict 语义未变。Final commit 后，从 exact final archive 独立
执行 remote default strict 两次；两次 canonical stdout 与两份 canonical strict attestation 必须分别 byte-identical，
stdout SHA-256 还必须与 `Balance-Calibration-Report-SHA256` trailer 精确相等。之后才在 clean live `main`、
`HEAD = final` 运行 materialization/phase/root gates；本机不运行 final full corpus。

完整 protected audit/replay 后才分类 live tree。Clean committed `HEAD` threshold-only red 且 `N < 12` 进入 step
`N+1`；`N = 12` 仍 red 或 selector 无严格改善则停止为 `balance_contract_unsatisfied`。Clean final-parent 只有在其 exact
archive 的一次 remote strict 和本机 full admission 通过后才进入 final materialization。Dirty 只在 step allowlist 且
clean-`HEAD` threshold-only red 时，在 sandbox 以同一 `--iteration=N` 重算、重新应用并要求完整 binary patch 等于
live patch；dirty 只在 final allowlist 且 parent remote strict 已 admission 时，重跑 writer/精确 removals、parent-report
injected candidate check 和上述 exact-patch/closure guards，不运行 full corpus。混合、范围外 bytes、无效 ancestry 或
不可重放 patch 一律停止为未知 dirty state。
...
物理主机不是 balance 语义或校准权威。所有者授权的辅助执行器可以承担 canonical 邻居证据和 default strict full
report 的并行评估，但必须保持本机为唯一 controller、admission 与 commit authority，并满足以下 proof-preserving
合同。`pnpm verify:balance` 仍是唯一 strict 语义；remote strict mode 只是从 exact archive 执行该 default path：
...
Calibration semantic value 是 aggregate evidence，而不是 opaque score：它包含 current evaluation、按 canonical 顺序
评估的每个合法邻居（或以首个零缺口结束的精确前缀）以及 claimed selection。Strict semantic value 是单行
`PoC balance report <canonical-json>\n`，其中 exact report 只有 `deficit` 与完整 `evaluation`，且 admitted/derived
deficit 必须为零。每个 evaluation 都包含完整 1..1000 aggregate metrics 与 candidate-specific counterfactuals。物理主机
和 worker 调度不进入这些 bytes；本机验收通过重新编码与严格 Story admission 证明 bytes 的唯一形状、合法输入集合、
确定 selector 或 passing full report，不通过在第二台机器重复完整 corpus 来定义结果。
...
- 输入必须是目标 clean commit 的 exact `git archive`，并在两端校验 source commit/tree、archive SHA-256、`pnpm-lock.yaml`
...
- Calibration controller 必须用同一 exact archive 的 Story-local evidence codec 对 semantic value 重新 canonicalize 并要求 byte-identical，严格验证 schema、iteration/current values、aggregate evaluation、canonical neighbor identity/order/completeness、首零短路和 supplied selection，再独立重算 deficit/selector；strict controller 必须验证独立 strict result contract、单行 report framing/canonical bytes、Story full-report schema/identity/count/counterfactual admission 和 derived zero deficit；两者都不调用真实完整 corpus evaluator；
...
- Calibration semantic stdout 继续只有 canonical calibration value；strict semantic stdout 继续只有 canonical full
  report line。两种模式使用独立 result/execution contract IDs。分离的 calibration execution attestation 记录
  source/toolchain/package identity、platform/arch、available parallelism、worker count、iteration 与
  before/after/evidence digests；strict execution attestation 记录同一 identity、固定 corpus、report/evaluation digests。
  两者都不得记录 SSH address、private IP、绝对路径、时间、耗时、主机名或 shard 完成顺序。
...
Phase 0 store 执行 recovery-only offline frozen install，但不得调用真实完整 corpus evaluator；所有本地/远端临时
sandbox 都在进程结束时清理或保留为 ignored/operator workspace，绝不进入 Git execution state。Final-parent report 与
strict attestation 必须在 materialize dirty candidate 前保存到仓库外；若恢复时缺失，只能从同一 exact parent 在可验证的
clean attached-`main` controller checkout 重新生成，不得从 dirty candidate、不同 commit 或缓存 evaluation 猜测。
...
Controller grammar 保持两个互斥的 direct modes。Calibration 继续使用
`--iteration=N [--prior-after-sha256=...] --host=... --remote-root=Workspace --workers=1..64
--attestation-out=<absolute-path>`；strict 没有 package alias，精确使用
`node scripts/run-poc-balance-remote.mjs --strict --host=... --remote-root=Workspace --workers=1..64
--attestation-out=<absolute-path>`，拒绝 iteration/prior 参数。Remote strict result 的 contract ID 是
`project-tavern.poc-balance-remote-strict-result.v1`，只接受 status 0，并且 archived runner 的精确 semantic invocation 是
`node scripts/verify-poc-balance.mjs --workers=<workers>`，不带 `--calibrate`、`--qualify-provisional` 或另一 mode flag。

分离 calibration attestation 的精确 v1 形状为：
...
Strict result 与 calibration result 不共享 contract ID；分离 strict attestation 的精确 v1 形状为：

```ts
interface PocBalanceRemoteStrictExecutionAttestationV1 {
  readonly schemaRevision: 1;
  readonly contractId: "project-tavern.poc-balance-remote-strict-execution.v1";
  readonly sourceCommit: string;
  readonly sourceTree: string;
  readonly sourceArchiveSha256: string;
  readonly pnpmLockSha256: string;
...
若辅助执行器不可用，本机完整 selector 仍是 calibration step 的语义等价 fallback，不构成设计阻塞；final strict
冻结则必须等待或恢复一个满足上述 exact-input/canonical-output 合同的授权执行器，不能把 candidate injected replay
提升成 full-corpus evidence。

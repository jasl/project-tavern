...
Phase 6 数值校准可以使用一个由项目所有者明确授权的远端**计算执行器**。该执行器只接收 exact clean
source archive，在固定工具链和 frozen lockfile 下运行 Story-local 只读 balance evaluator；它不接收、构建、启动、
验证或发布游戏 Artifact，也不提供 hosting、runtime service 或 remote smoke。执行器返回 canonical aggregate
evidence；本地主控从同一 frozen archive 加载 Story-owned codec/admission/selector，严格重编码并验证 schema、完整合法
邻居集或恰好结束于首个零缺口的 canonical 前缀，独立重算 deficit/selector，并核对 source、toolchain、input、output
摘要和相邻 step digest chain。该验收不再次执行昂贵的 current/selected 全量模拟。因而该有限计算外包仍属于本地
工程轨道的验证实现细节，不建立第五条轨道，也不构成下文的远端分发。

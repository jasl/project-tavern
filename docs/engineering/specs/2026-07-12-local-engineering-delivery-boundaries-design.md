...
验证或发布游戏 Artifact，也不提供 hosting、runtime service 或 remote smoke。执行器结果只有在本地主控严格重算
selector、逐字节核对当前点并完整复算最终选中候选后才可进入 calibration-step commit。因而该有限计算外包仍属于
本地工程轨道的验证实现细节，不建立第五条轨道，也不构成下文的远端分发。

...
- [`2026-07-15-typed-state-store-v2-design.md`](2026-07-15-typed-state-store-v2-design.md)：当前 Phase 2–6 Goal 完成后的 typed StateStore 演进边界；它不改变本 Goal 的 v1 Snapshot、Query、事务或持久化 ABI；
...
- Base 先验证 Save envelope/integrity/provenance；Story reference validator 只接收 Gameplay State，Story invariant validator 只接收深只读 `{ state, commandSequence }` 窄视图，以验证 State 内的 sequence 引用；二者都不能读取或改写完整 Snapshot envelope、RNG、RunIntegrity 或 provenance；

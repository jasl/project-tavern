...
- The current Goal defers full 1..1000 balance calibration to the start of Phase 6, before any release/Artifact work. Phase 4B freezes deterministic balance infrastructure, a fast smoke corpus and provisional golden/Save baselines; Phase 5 consumes those exact bytes read-only. Phase 6 then calibrates the frozen thresholds, synchronizes authoritative balance data/documentation, proves one clean final-parent exact archive by remote default strict, explicitly rebinds the reviewed Save fixture provenance to the calibrated live identity, regenerates and technically reviews every affected golden/Save/digest byte, checks the dirty candidate only by injecting that parent-admitted evaluation through candidate default-strict code with zero real full-corpus evaluator/import calls, and proves the exact final archive by two byte-identical remote strict runs before Artifact work. This changes no phase order, keeps `pnpm verify:balance` as the sole strict semantic gate, and does not weaken the final balance contract.
...
pnpm --filter @project-tavern/story-poc calibrate:balance:remote  verified auxiliary full-corpus selection plus local admission
...
Remote strict is a Phase-6-plan-private direct invocation of `scripts/run-poc-balance-remote.mjs --strict`; it creates no
package alias or third strict semantic gate. It executes the default `pnpm verify:balance` semantics from an exact clean
archive and returns evidence for local Story admission.
...
Before Phase 6 plan Task 1 or any release/Artifact implementation or build, establish the balance-freeze checkpoint. Run the frozen seeds 1..1000 × six strategies and counterfactuals, apply only the deterministic finite calibration procedure, and update `docs/poc/balance-v0.md` and `game/stories/poc/src/content/balance.ts` together. Current-point/neighbor aggregate evaluation and final strict reports run on an owner-authorized 1..64-worker auxiliary executor from an exact clean archive. The local controller accepts no opaque score: calibration mode canonicalizes and strictly admits the complete aggregate evidence, verifies the legal-neighbor set or first-zero canonical prefix, independently recomputes deficit/selection, and binds exact source/toolchain/input/output digests plus the prior-step before→after chain; strict mode re-encodes and Story-full-admits the complete report, derives zero deficit, and emits an independent canonical strict attestation. After one clean final-parent remote strict pass, project its live Save provenance, rebind only the reviewed frozen scalars changed by accepted steps, and regenerate affected golden/Save/digest outputs through the existing explicit writers. Repeat the Phase 4B full diff/hash technical review and exact-staging contracts, prove reference command bytes unchanged, and check the dirty candidate without a full corpus by injecting the parent-admitted evaluation through candidate `runPocBalanceCliV1({ args: [] })`; require zero real full-corpus evaluator/import calls, one exact injected evaluation, byte-identical stdout, the exact planned patch, an empty projection onto protected balance/evaluator/codec/strategy paths, and only the exact provisional removals in the verify CLI/tests. Commit the clean final bytes, then run remote strict twice from the exact final archive and require stdout/strict-attestation byte identity plus exact report-trailer equality. If no improving legal neighbor exists, stop before Artifact work; never lower a threshold, alter the six strategies, or relabel provisional bytes as final.
...
- [ ] After accepted exact-archive remote strict evidence, run the remaining local phase/root/release verification and record source SHA, manifest digest, browser matrix and clean status without repeating the full corpus locally.
...
- `pnpm verify:balance` semantics pass the complete frozen 1..1000 corpus; the exact final archive has two independent remote strict runs with byte-identical canonical stdout and strict attestations, the stdout SHA-256 matches the final trailer, the reviewed Save fixture provenance matches the calibrated live resolved identity, and the golden/Save/digest bytes regenerated after that calibration are the same bytes consumed by release verification and `dist/poc`;

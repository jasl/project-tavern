...
- The current Goal defers full 1..1000 balance calibration to the start of Phase 6, before any release/Artifact work. Phase 4B freezes deterministic balance infrastructure, a fast smoke corpus and provisional golden/Save baselines; Phase 5 consumes those exact bytes read-only. Phase 6 then calibrates the frozen thresholds, synchronizes authoritative balance data/documentation, explicitly rebinds the reviewed Save fixture provenance to the calibrated live identity, regenerates and technically reviews every affected golden/Save/digest byte, and only then begins Artifact work. This changes no phase order and does not weaken the final balance contract.
...
Before Phase 6 plan Task 1 or any release/Artifact implementation or build, establish the balance-freeze checkpoint. Run the frozen seeds 1..1000 × six strategies and counterfactuals, apply only the deterministic finite calibration procedure, update `docs/poc/balance-v0.md` and `game/stories/poc/src/content/balance.ts` together, then project the final parent's live Save provenance, rebind only the reviewed frozen scalars changed by those accepted steps, and regenerate affected golden/Save/digest outputs through the existing explicit writers. Current-point and neighbor aggregate evaluation may run on an owner-authorized 1..64-worker auxiliary executor from an exact clean archive. The local controller accepts no opaque score: it canonicalizes and strictly admits the complete aggregate evidence, verifies the legal-neighbor set or first-zero canonical prefix, independently recomputes deficit/selection, and binds exact source/toolchain/input/output digests plus the prior-step before→after chain; it does not repeat the full corpus locally. Repeat the Phase 4B full diff/hash technical review and exact-staging contracts, prove reference command bytes unchanged, run local `pnpm verify:balance` twice with byte-identical passing metrics, run read-only golden/fixture gates, and commit the clean final bytes. If no improving legal neighbor exists, stop before Artifact work; never lower a threshold, alter the six strategies, or relabel provisional bytes as final.
...
- `pnpm verify:balance` passes the complete frozen 1..1000 corpus, the reviewed Save fixture provenance matches the calibrated live resolved identity, and the golden/Save/digest bytes regenerated after that calibration are the same bytes consumed by release verification and `dist/poc`;

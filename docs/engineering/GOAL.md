...
- Phase 6 可以把 canonical 邻居证据的只读计算交给所有者授权的辅助执行器：无零缺口候选时它包含完整合法集；一旦出现零缺口则必须恰好结束于 canonical 顺序的首个零缺口候选。只传 clean exact source archive，绑定 source/tree/archive/lock/materialization/package closure 与 exact Node/pnpm，显式使用 `1..64` workers，正式 run 从 fresh archive 进行 offline frozen install。`N = 0` current point 与每轮 selected candidate 都必须在本机完整复算并与远端 evaluation byte-identical；本机还要严格 admission 全部 evidence 并重算 selector。SSH host、路径和调度不进入 semantic evidence/attestation，远端不接收或运行 Artifact、Vite、Playwright、server、writer、remote smoke 或 distribution。

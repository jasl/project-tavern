...
- Phase 6 校准由零到十二个独立 calibration-step commits 与唯一 final balance-freeze commit 组成。每个 step commit 只应用一个确定候选及其权威文档/直接 expectations；golden、Save 和临时 qualifier 保持不变。Clean final-parent exact archive 的一次 remote strict 和本机 full admission 通过后，final candidate 才把冻结的 Save fixture provenance 中实际漂移的字段机械重绑定到同一 final parent 的 live resolved provenance，再移除 qualifier、重生成并完整复核 golden/Save/digest，且不再修改 balance 数值或直接 expectations。Dirty candidate 通过 parent-evaluation injected default-strict 等价检查后提交，exact final archive 再以两次 remote strict 建立决定性证据。
- Phase 6 可以把 current point/canonical 邻居的 aggregate evaluation，以及 clean final-parent/final exact archive 的 default strict full report，交给所有者授权的 `1..64`-worker 辅助执行器。Calibration evidence 在无零缺口候选时包含完整合法集，一旦出现零缺口则恰好结束于 canonical 顺序的首个零缺口候选；strict mode 只接受 default `pnpm verify:balance` 的 passing canonical full report。两种模式都只传 clean exact source archive，绑定 source/tree/archive/lock/materialization/package closure 与 exact Node/pnpm，并从 fresh archive 进行 offline frozen install。本机 controller 不重复执行完整 corpus；calibration mode 必须 canonical re-encode、严格 schema/admission、核对 iteration/current values、验证 complete-neighbor-set 或 first-zero canonical prefix、独立重算 deficit/selector，并绑定 evidence/before/after/attestation digests；strict mode 必须重编码并 Story-full-admit exact report、独立重算 claimed deficit 为零，并写入使用独立 strict result/execution contract ID 的 canonical attestation。`N = 0` 直接接受 admitted current evaluation 作为 before；`N > 0` 还必须让本轮 before digest 精确等于上一 accepted step 的 after digest。任何 framing、identity、admission、prefix、selection、report 或 digest-chain mismatch 都禁止继续。SSH host、路径和调度不进入 semantic evidence/attestation，远端不接收或运行 Artifact、Vite、Playwright、server、writer、remote smoke 或 distribution。
- Phase 6 entry 紧邻 accepted Phase 5C checkpoint：从该 checkpoint 到 final（尚无 final 时到 `HEAD`）的每个 first-parent commit，不论 path，都必须且只能分类为 trailers/完整 binary patch 可重放且 index 严格连续的 step、带 `Balance-Calibration-Repair: true` 的闭集 repair，或唯一 final；任何无分类/多分类 commit 都使历史无效。Repair 的两个 subtype 互斥：Task 10 executable-owner repair 的 diff 必须至少修改一个 Task 10 非权威文档的 executable/test/package path；calibration/finalization authority repair 的非空 diff 必须完全落在本节七份权威文档（本文件、Roadmap、local-engineering delivery-boundaries design、balance-lab design、Phase 4B plan、Phase 6 plan、`docs/poc/balance-v0.md`）内。后者不得修改 executable、balance/direct expectations、qualifier、golden、Save、command/tooling fixture 或 lockfile。允许 `N = 0` 且始终要求 `N <= 12`。final 必须晚于最后一个 step/repair；final 后的普通 Phase 6 task commit 可以存在，但其显式 first-parent diff 不得再触及 step/final protected paths。
- 每个历史 step 都必须在其 parent 的 clean sandbox 重跑本机 selector，或从该 parent 的 exact archive 重建远端 canonical aggregate evidence 并执行上述本机 admission/selector/digest-chain 验收；两种路径都校验 evidence SHA-256 与全部 trailers，并重建包括文档和直接 expectations 在内的完整 patch，只有 `git diff --binary` byte-for-byte 相同才接受。若 executable-owner repair 发生在 `N > 0`，必须用 repaired evaluator 从 Phase 5C baseline 顺序重放全部旧 steps；但只改变 remote controller 验收/编排、且不改变 evaluator/runner/selector/evidence codec 或其语义输入的 controller-only orchestration repair，必须把 repaired controller 作为每个原 step parent exact archive 之外的 admission 层运行，保留原 archive identity，不得把 controller repair overlay 进历史 archive。增加 strict full-report admission/finalization 编排的第二个 executable-owner repair 同样在历史 archive 外运行 repaired calibration controller，必须重放原 step evidence/trailers/full patch，并证明其 final-parent→candidate 保护投影对 balance、evaluator、codec、strategies 和 default strict 语义为空。若 authority-only calibration/finalization repair 发生在 `N > 0`，先证明该 repair 的 parent→commit patch 对 calibration executable/import closure 的投影为空，再在每个历史 step 的原 parent/exact archive 上重放；不得把无关 authority bytes overlay 到旧 archive 后伪造新的 source-archive identity。任一 evidence/trailer/full-patch 不同即停止为 `balance_calibration_history_invalidated`，不得自动重写或 rollback。`N = 0` 的 repair 可在 owner gates 通过后继续。
- 只有带 `Balance-Calibration-Final: true`、`Balance-Calibration-Steps: N` 与最终 report SHA-256 trailer 的 clean accepted commit 才是最终平衡冻结 checkpoint。clean/no-final 首次收口先从 clean non-detached live `main` 的 exact final-parent archive 在辅助执行器运行一次 default strict，并在本机对 canonical report 完成 full admission；随后才投影并审阅 Save fixture provenance、只重绑定与 accepted balance steps 一致的冻结字段，再 materialize writer/removal candidate。final-parent sandbox 独立重复 provenance 重绑定、writers 与精确移除 provisional report/assertion/CLI branch/tests 与文档状态文字，并以完整 binary patch 对比。Dirty candidate 不运行 full corpus：它把 parent 已 admission 的 evaluation 注入 candidate `runPocBalanceCliV1({ args: [] })`，要求真实 full-corpus evaluator/import 零调用、injected provider 恰好返回该 evaluation、candidate Story full admission 后 stdout 与 parent report byte-identical；final patch 对 balance/evaluator/codec/strategy protected paths 的投影必须为空，verify CLI/tests 只能出现 exact provisional removals。提交 final 后，再从 exact final commit archive 在辅助执行器独立运行 default strict 两次，要求 canonical stdout 和 strict attestation 分别 byte-identical，且 report SHA-256 精确匹配 final trailer；退出后回 live `main`、`HEAD = final` 执行 materialization/phase/root gates。本 Goal 不再要求本机执行 final full corpus。
...
所有 dirty recovery、历史 step/final replay 的本机 selector/writer/patch/admitted-candidate 重建都使用同一个可执行 clean-commit sandbox；`target_commit` 按场景取 `HEAD`、step parent、final parent 或 final commit。Clean final-parent/final 的 full-corpus strict 例外从对应 exact archive 在辅助执行器运行，不在该本机 sandbox 重复：
...
该 sandbox install 是 recovery-only、offline、frozen-lockfile 且只写临时 worktree；执行者先选择 Phase 0 materialized PATH（当前 checkpoint 为 `/opt/homebrew/bin`），live 与 subshell 的 Node/pnpm 断言均通过后才可取 store 或产证据。不得访问 registry 或改变 live tree。辅助计算执行器的一次性隔离 provisioning 与正式 offline evidence run 由 balance-lab authority 单独约束，不改变这一本机 sandbox 合同或 ignored materialization attestation。若 final 不存在，clean committed `HEAD` 的 threshold-only red 且 `N < 12` 时运行本机 selector，或运行带本机严格 canonical/schema/admission/complete-prefix/selector/digest-chain 验收的 `calibrate:balance:remote --iteration=N`；clean final-parent 则必须先完成一次 exact-archive remote strict full admission，才进入上述 live-candidate/final-parent-replay 流程。`N = 12` 仍红或 selector 无严格改善时停止为 `balance_contract_unsatisfied`。dirty step 仍须从 clean-`HEAD` sandbox 重建 selector/full patch；dirty final 只使用 parent admitted evaluation 的 candidate default-path admitted replay 与 writer/removal full-patch/closure guards，不在 dirty bytes 上执行 full corpus。混合、范围外或不可重放 bytes 按未知 dirty 停止。最终 `pnpm verify`、`pnpm verify:release`、可复现 Artifact 和 Definition of Done 都消费 final commit 后的同一组权威 bytes。

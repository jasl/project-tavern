...
- Phase 6 校准由零到十二个独立 calibration-step commits 与唯一 final balance-freeze commit 组成。每个 step commit 只应用一个确定候选及其权威文档/直接 expectations；golden、Save 和临时 qualifier 保持不变。严格完整 gate 通过后，final commit 先把冻结的 Save fixture provenance 中实际漂移的字段机械重绑定到同一 final parent 的 live resolved provenance，再移除 qualifier、重生成并完整复核 golden/Save/digest，且不再修改 balance 数值或直接 expectations。
- Phase 6 可以把 current point 与 canonical 邻居的完整 aggregate metrics/counterfactual evaluation 交给所有者授权的辅助执行器：无零缺口候选时 evidence 包含完整合法集；一旦出现零缺口则必须恰好结束于 canonical 顺序的首个零缺口候选。只传 clean exact source archive，绑定 source/tree/archive/lock/materialization/package closure 与 exact Node/pnpm，显式使用 `1..64` workers，正式 run 从 fresh archive 进行 offline frozen install。本机 controller 不重复执行完整 corpus；它必须对远端 semantic bytes 做 canonical re-encode、严格 schema/admission，核对 iteration/current values，验证 complete-neighbor-set 或 first-zero canonical prefix，独立重算 deficit/selector，并绑定 evidence/before/after/attestation digests。`N = 0` 直接接受 admitted current evaluation 作为 before；`N > 0` 还必须让本轮 before digest 精确等于上一 accepted step 的 after digest。任何 framing、identity、admission、prefix、selection 或 digest-chain mismatch 都禁止应用候选。SSH host、路径和调度不进入 semantic evidence/attestation，远端不接收或运行 Artifact、Vite、Playwright、server、writer、remote smoke 或 distribution。
...
该 sandbox install 是 recovery-only、offline、frozen-lockfile 且只写临时 worktree；执行者先选择 Phase 0 materialized PATH（当前 checkpoint 为 `/opt/homebrew/bin`），live 与 subshell 的 Node/pnpm 断言均通过后才可取 store 或产证据。不得访问 registry 或改变 live tree。辅助计算执行器的一次性隔离 provisioning 与正式 offline evidence run 由 balance-lab authority 单独约束，不改变这一本机 sandbox 合同或 ignored materialization attestation。若 final 不存在，clean sandbox strict pass 进入上述 live-candidate/final-parent-replay 流程；threshold-only red 且 `N < 12` 时运行本机 selector，或运行带本机 strict canonical/schema/admission/complete-prefix/selector/digest-chain 验收的 `calibrate:balance:remote --iteration=N`。`N = 12` 仍红或 selector 无严格改善时停止为 `balance_contract_unsatisfied`。dirty step/final 只有在对应 clean-`HEAD` sandbox 重建的完整 `git diff --binary` 与 live dirty patch 精确相等时才可继续；混合、范围外或不可重放 bytes 按未知 dirty 停止。最终 `pnpm verify`、`pnpm verify:release`、可复现 Artifact 和 Definition of Done 都消费 final commit 后的同一组权威 bytes。

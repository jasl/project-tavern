...
获选素材使用以下结构；未获选候选保留在被忽略的本地工作目录，不进入 Git：
...
每个获选素材的 `provenance.json` 至少包含：

```json
{
  "assetId": "background.tavern.main.day",
  "sourceType": "ai_generated",
  "generator": {
    "service": "OpenAI Image Gen",
    "surface": "Codex built-in image_gen",
    "model": "undisclosed-by-tool",
...
工具未披露模型时记录 `undisclosed-by-tool`，不得猜测。`termsReview.status` 未变为 `approved` 的素材可以作为 `art-source` 候选和 Developer preview，但不能进入 Player runtime manifest 或 Pages artifact。
...
- 未获选候选、商业参考图与 `references/` 不进入仓库、生成输入或构建。

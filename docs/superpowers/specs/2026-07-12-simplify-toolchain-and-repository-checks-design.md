...
**日期：** 2026-07-12
**状态：** Phase 1 清理已完成；由后续交付边界作有限修订
**适用范围：** Phase 1 工具链/静态治理清理的历史设计，以及未被后续规格替代的项目法律文件原则

> [`2026-07-12-local-engineering-delivery-boundaries-design.md`](2026-07-12-local-engineering-delivery-boundaries-design.md)
> 和其 Phase 0 物化计划为一次有边界的 Phase 2–6 Goal 重新记录精确本机 Node/pnpm checkpoint、`.node-version` 与 `packageManager`。这只是提前物化外部输入、避免长任务中途联网的执行凭据，不把补丁版本重新定义为项目永久兼容性或代码正确性条件。本文的 Player/Developer 术语也只保留为 Phase 1 as-built 记录。
...
- Phase 1 清理删除了 `.node-version`；Phase 0 可以为当前有界 Goal 重新创建它，Goal 完成后的长期保留/升级按物化计划处理。
- Phase 1 清理删除了根 `package.json.packageManager`；Phase 0 可以为当前有界 Goal 重新记录精确 pnpm，且不得据此缩窄 `engines` 兼容性声明。
- 删除 `.npmrc` 的 `engine-strict=true`；`engines` 是兼容性说明，不是安装阻断器。
- 删除 `verify:toolchain`、`scripts/verify-toolchain.mjs` 及其测试。
- 普通项目验证不把实际 Node.js/pnpm 补丁版本当作软件正确性；Goal 启动前的 `verify:materialization` 例外地核对已批准 checkpoint，TypeScript 仍由 frozen lockfile 决定。
...
- 直接执行的 `.mts` 工具统一使用 Node 的 `--experimental-strip-types`；其完整导入闭包保持 type-erasable，不使用 enum、namespace、parameter property 或其他需要 transform 的语法，从而兼容支持范围内不提供 transform CLI 的 Node 版本。
...
- 根 `engines` 只声明 Node.js 和 pnpm 最低兼容版本；Phase 0 的精确环境闸门只约束当前 Phase 2–6 Goal，不改变对项目消费者的兼容性声明。

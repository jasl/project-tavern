# Project Tavern Phase 2+ 游戏运行时架构修订

日期：2026-07-12

状态：已确认；Phase 2–6 的权威架构输入

适用基线：Phase 1 完成提交 `4e9c2bd5b06f3cc6f338d30ff43bc6e0188f74d2`

## 1. 权威范围

本文记录 Phase 1 完成后确认的架构调整，覆盖 Phase 2–6 的术语、Story 所有权、Application Port、Debug/Automation、Input 和构建边界。
...
这些机制应迁移和复用，不应重写。Phase 1 的 Sandbox walking story 迁移为 E2E Story；当前空 `stories/e2e` stub 被替换，空 `stories/demo` stub 迁移为 `stories/poc`。
...
Phase 2 必须完成以下迁移，不导出旧别名：

```text
GameProfileTypeMapV1       → GameSimulationTypeMapV1
GameProfileTypeWitnessV1   → GameSimulationTypeWitnessV1
GameProfileV1              → GameSimulationV1
defineGameProfile          → defineGameSimulation
CommandCoordinatorV1       → GameCommandExecutorV1
profile.coordinator        → gameSimulation.commandExecutor
StorySimulationFacet.createProfile
                           → StorySimulationFacet.createGameSimulation
ResolvedStoryV1            → ResolvedGameV1
resolved.profile           → resolved.gameSimulation
EngineSessionV1            → GameSessionV1
createEngineSessionV1      → createGameSessionV1
EngineSessionRuntimeControlV1
                           → GameSessionRuntimeControlV1
GameModule*                → GameplayModule*
DomainFact*                → GameplayFact*
StatelessGameplayModule.services
                           → StatelessGameplayModule.capabilities
```

`GamePackageV1`、`StoryDefinitionV1`、`resolveGamePackageV1`、`ModuleId` 和 `StateSlotId` 保留。

具体 PoC 类型必须带 `Poc` 前缀，例如：

```text
GameStateV1         → PocGameStateV1
GameCommandV1       → PocGameCommandV1
StoryRulesV1        → PocRulesV1
EngineQueriesV1     → PocGameQueriesV1
DemoProfileTypesV1  → PocGameSimulationTypesV1
```
...
- `packages/assets` 继续提供共享素材合同和经选择的跨 Story 资产；Story 可拥有本地素材。

现有目录迁移为：

```text
stories/sandbox → stories/e2e
stories/demo    → stories/poc
```
...
体力、具体属性、NPC、人物关系、任务、对话、酒馆、设施、探索和 PoC Effect 联合均属于 `stories/poc`，不属于 Base。
...
interface GameSimulationV1<
  ...
  TView,
  TCommandExecutor,
  TDebugCommandExecutor,
> {
  readonly contractRevision: 1;
  readonly modules: readonly GameplayModuleBindingV1[];
  readonly commandExecutor: TCommandExecutor;
  readonly debugCommandExecutor: TDebugCommandExecutor;
  createQueries(state: DeepReadonly<TState>): TQueries;
  projectGameView(queries: TQueries): TView;
}
```
...
Phase 1 的 `ResolvedStoryV1` 没有保留 source SceneGraph，应用入口又手工组装 UI。Phase 2 修复为：
...
Resolver 顺序保持：define → fresh Patch registries → Hotfix → revoke writes → materialize Programs once → validate/deep-freeze → create GameSimulation once → resolve assets/identities → freeze ResolvedGame。

Resolver 的 fresh-definition determinism 分两层验证：可序列化 data facets 使用 Canonical JSON 比较；Rule/Resolver/lifecycle 等 executable capabilities 必须是模块级稳定函数引用，或带稳定 provider ID/source digest 的受控 descriptor，并按允许键、引用稳定性与 import-closure digest 校验。不得把函数放进 JSON 投影后因其被丢弃而误判为相同，也不得接受每次 `define()` 新建的匿名/nondeterministic executable provider。

SceneGraph 必须来自 resolved Presentation，应用入口不能再从 source Story 重取或创建第二份图。改变 GameplayModule 集合、State Schema、Command/Fact 联合、控制流或 Rule 必须改变 simulation/state-contract identity；纯文本、素材和布局只改变 presentation/application identity。

默认 Story/Headless import closure 中的 SceneGraph 是 Node 可直接 type-strip 导入的 `.ts` 数据描述符：只包含稳定 renderer ID、layout/slot descriptor 和 Strict JSON presentation data，不包含 JSX、React component、函数或浏览器对象。Story 的 Web-only `.tsx` renderer registry/contribution 在 application closure 中把 renderer ID 解析为组件；它影响 application identity，但不能反向进入 GameSimulation、默认 Story entry 或 Headless closure。

## 8. GameSession 与 RunIntegrity

GameSession 继续使用 Phase 1 的单 FIFO。Gameplay dispatch、合法 Save load/import、生命周期替换、replayable DebugCommand、fixture/DebugBundle anchor 和 Semantic preview 的权威读取共享同一串行化边界。Semantic preview 在轮到自己的队首时取得最新 Gameplay State，再创建 Queries；不能在调用时缓存旧 Snapshot 后越过已排队的 dispatch。
...
原有 PlayerPersistencePort 等低权限子端口名称可以保留。删除 `DeveloperApplicationPort`、独立 Developer application root、Developer Artifact 和以 bundle absence 作为安全边界的测试。
...
现有 `StoryDevelopmentEntryV1` 的角色改为同一 Artifact 中可延迟加载的 Story tooling。Phase 2 将其重命名为 `StoryToolingEntryV1`/`StoryToolingSupportV1`；不建立独立 build。fixtures、开发备注和把受控表单输入构造成已声明 DebugCommand 的 UI adapter 可以在 capability 开启时动态导入。Replayable DebugCommand 的 Schema、验证、owner proposal 和 attempt 语义不得放在 lazy tooling 中；它们属于 resolved GameSimulation，并进入 `simulationDigest`。
...
```ts
interface SemanticGamePortV1<TView, TActionDescriptor, TInvocation, TPreview, TResult> {
  observe(): DeepReadonly<TView>;
  ...
  waitForIdle(afterRevision?: NonNegativeSafeInteger): Promise<DeepReadonly<TView>>;
}
...
Story 特化的 `TInvocation` 必须是 strict、可序列化的闭合联合；它不能携带 callback、任意 State 路径或脚本。Semantic action descriptor 至少包含稳定 ActionId、TextId、enabled、按声明顺序排列的禁用原因和构造 invocation 所需的受控选项。
...
`SemanticGamePortSourceV1` 是 Base/Application 在 bootstrap 时使用的内部组合输入，不通过 `GameApplicationPort` 暴露给 UI、内容、Hotfix 或 Automation，也不接受运行时提供的任意 reader。Base 的 factory 只把 Gameplay State 交给已解析 Story-owned `createQueries`，不会把完整 Snapshot、RunIntegrity、RNG 或 sequence 传入 Semantic callback。`getAuthoritativeRevisionToken()` 只提供不可解构的引用恒等 token，使 factory 能识别 envelope 替换而不暴露任何字段。`reportSubscriberFailure` 把 Semantic listener 异常转发到与 GameSession observer 相同的有界 runtime-failure sink；factory 必须继续通知其余 listener，sink 自身异常也被隔离。`revision` 只在该权威 token 改变时递增；busy/idle 等 status 改变可以 publish subscriber，但不能伪造一次 Gameplay revision。Lifecycle/load/anchor 造成的权威替换也必须发布。React 通过 `subscribe`/`useSyncExternalStore` 消费不可变投影，可以拥有更丰富的视觉 `GameView`，但其操作、禁用状态和原因必须来自与 SemanticGameView 相同的 GameQueries。浏览器 E2E 同时验证 Port 和真实 DOM，不能用 Port 替代可访问性测试。
...
- `packages/ui/src/input` 提供 InputAction、InputContext 和 InputRouter；
- `apps/web/src/input` 提供 Pointer Adapter；
- 使用 Pointer Events 同时兼容鼠标、触摸和触控笔；
- 普通交互继续使用原生 `<button>`、表单和焦点行为；
- Overlay、Narrative、System、Debug 按显式 Context 优先级阻止输入穿透；
...
七日酒馆的具体 Gameplay 全部位于 `stories/poc`：
...
Phase 1 的 Host record-store、严格 Save envelope、CommandLog 和单 FIFO 原则保留。后续必须满足：
...
当前只有开发基线，无向前兼容承诺。Phase 2 ABI 迁移后显式重生成 E2E fixture；不编写旧 Sandbox Save 迁移器。

## 16. UI 与 Debug 工具

- `@project-tavern/ui` 提供中性的 Shell、Stage、Overlay、VN、Input、DevDock framework 和可访问组件；
- PoC HUD、关系、任务、营业、设施和 Scene renderers 位于 `stories/poc/src/presentation`；
- E2E 拥有自己的最小 Scene；

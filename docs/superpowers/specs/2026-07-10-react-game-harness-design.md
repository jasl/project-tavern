...
- `apps/web` 是通用 Loader/Web Host/mount library，只负责装载前、失败和安全模式 UI；Story-owned Web application root 把自己的 StoryEntry 作为显式参数传入，进入游戏后由 Story 控制；
...
- 经隐私裁剪且满足固定大小上限的诊断数据；内容类别与编码后字节大小由导出 UI 在提交下载前派生展示，不新增未版本化的 Bundle 字段。

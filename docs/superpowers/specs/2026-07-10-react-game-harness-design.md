...
- `apps/web` 是通用 Loader 和 Web Host，只负责装载前、失败和安全模式 UI；进入游戏后由 Story 控制；
...
- 将 Demo Story 静态构建到 Web Loader，并部署到 GitHub Pages 供朋友试玩；
...
- `defineGamePackage`、`defineStory`、`defineGameModule`、`definePatchSlot`、`defineHotfix` 等无副作用 authoring helper；
- Snapshot、Command、Fact、错误、identity、Strict JSON 和 Canonical JSON 基础合同。

建议子路径：

- `@project-tavern/base/runtime`：EngineSession、事务、RNG、Story/Hotfix resolver、Persistence、Replay、Diagnostics 和 Loader service；
- `@project-tavern/base/testkit`：Module/Story/Hotfix contract suite、builders、deterministic driver 和断言。
...
- Story 可以依赖 Base、UI、Modules 和 Assets，但不能导入 App；
- App 依赖 StoryEntry 和 Host/runtime，不反向定义游戏内容；
- E2E Story 可以组合公开的真实 Modules，但不能导入 Demo Story 内容或私有实现；
- 生产代码、测试、生成脚本和构建不得读取 `references/`。

ESLint import restrictions、package `exports`、TypeScript project references 和 cycle 检查共同执行这些边界。

## 8. Loader、Host 与启动生命周期

`apps/web` 是通用 Loader。第一版通过构建配置静态导入一个 StoryEntry；这不妨碍未来把同一 Story 构建成独立 `entry.mjs + manifest + assets`。Story 的默认入口只含 Player runtime；fixtures、预览和开发备注位于独立 `./development` export，只有 Developer build 的静态 alias 可以导入。
...
```text
WebHost / ElectronHost
  → Generic Loader
  → StoryEntry.define()
  → Story 组合 GameProfile、SceneGraph、内容与素材
  → 暴露 PatchSurface
...
Host Adapter 至少抽象：KV/IndexedDB 存储、文件选择与下载、当前时间戳 metadata、导航/退出意图、日志和平台信息。确定性模拟层拿不到真实时间、存储、网络、DOM 或平台对象。
...
Story 在 `define()` 阶段选择 Module 并构造 `GameProfile`；`ResolvedStory` 冻结后，当前 EngineSession 不得安装、移除或替换 Module。变更 Module 集合需要重新 bootstrap，并产生新的 `simulationDigest`。

每个 Module 必须声明：

- 稳定 module ID 和合同 revision；
- 自己拥有的零个或多个状态路径；
- State Schema、初始值、局部不变量；
- 命令、查询、只读 port、owner-scoped proposal/apply capability；
...
- `id`、正整数 `revision` 和构建生成的 base Story digest；
- GameProfile 与所需 Module；
...
第一版 `apps/web` 静态组合 `stories/demo`。未来可增加 `story:pack` 输出独立分发目录，但不在首轮实现浏览器内依赖解析或包商店。
...
DebugBundle 默认不记录绝对路径、浏览器历史、任意存储内容或未选择的文件；导出前展示内容摘要和大小，不自动上传。
...
Base 暴露由五个窄子端口组成的泛型 `PlayerApplicationPort`：不可变 RuntimeViewModel 的 `getCurrent/subscribe`、类型化 command dispatch、显式 bootstrap input 的 create/restart lifecycle、persistence/lease，以及只读 diagnostics。Lifecycle 只建立 sequence-0 新锚点，不等同于 Demo 的 `run.start` Command。Persistence 只允许玩家写 `quick | manual`，同时提供四槽 list/status/load/clear/export、Save JSON import、无需 IDB 的 current-Snapshot export，以及 lease 请求交接/批准/接管/释放；Auto 两槽只能由运行时策略写入。它不暴露 Snapshot、EngineSession、Module owner capability 或任意 setter。
...
`@project-tavern/ui` 定义泛型 UI contribution contract：Story/Profile 静态注册唯一 Scene ID、Overlay ID、HUD slot、GameSymbol provider 和对应 renderer。Renderer 只接收其声明的只读 ViewModel slice 与 Player/Developer port；`@project-tavern/ui` 不读取 Demo command/state 类型。Demo 的具体 `DebugUiContextV1`、Overlay ID 和 Action ID 是该泛型 envelope 的一次 specialization，不属于 UI Base ABI。
...
构建生成不可变 `ResolvedAssetManifest`。未知 ID、usage 不匹配、缺少 slot/fallback、非法 sealed 覆盖、尺寸或完整性错误使 Story validation 失败；运行时 fetch/decode 失败则使用已经校验过的 code-native fallback，并记录 AssetLoadError。

...
内核服务于酒馆经营、关系养成和文字冒险。只有出现至少两个真实用例时才抽取通用机制。首版不引入 ECS、通用插件系统、任意规则语言、CQRS 或事件溯源。
...
1. `StartOpening` 校验计划、体力和基础资源，物化一个窄的、可序列化 `OpeningBaseline`（菜单、库存批次引用、参与者能力、设施/Modifier、需求输入和开始序号），不把整个 GameSnapshot 嵌入 workflow；
2. Scheduler 在固定检查点选择零个或多个营业事件；
3. 环境事件直接生成受控修正；交互事件进入 VN，选择结果通过领域命令写入 OpeningSession；
4. `FinalizeOpening` 调用 Story 的 `TavernSettlementRule` 生成 SettlementDraft；
5. 引擎校验库存消耗、容量、账本平衡、有限数值和 Story 输出 Schema；
6. Inventory、Actors、Status 与 Progression 分别应用其拥有的变化；
7. 全部通过后一次提交最终状态和详细账单。

`StartOpening` 和每条营业事件/选择命令都分别提交包含最新 ActiveWorkflow 的合法 Snapshot；各自只在其明示 checkpoint 消费 RNG。`FinalizeOpening` 内部的 SettlementDraft 校验与跨领域应用是单条原子事务。同一 OpeningSession 可以在已提交的随机事件或 VN 选项处 Quick Save，载入后从同一叙事游标和 RNG 位置继续。
...
- 一个滚动 Auto Save，内部保留 current 与 previous；
- 一个 Quick Save；
- 一个 Manual Save；
- JSON 导入、导出和清除。
...
- `saveRecords`：以 `[storyId, slotId]` 为键；
- `settings`：玩家和开发 UI 偏好；
- `diagnosticDrafts`：有界、可清理的本地故障草稿，不自动上传。
- `sessionLeases`：当前 Story 的 ownerId 与单调递增 fencingToken。

数据库名包含应用 ID 与当前 Story ID；Story ID 也保留在记录键和信封身份中，防止导出或未来迁移时失去来源。每个 SaveRecord 带单调递增的 `recordRevision`。所有写入在同一 readwrite transaction 内读取当前 revision、比较调用方的 expected revision、写入 candidate 并更新 current/previous。revision 不符时返回显式 conflict，不允许最后写入者静默覆盖。
...
背景以 2560×1600 母版制作，运行时优先导出 WebP；透明人物使用 PNG 或 WebP。首版不因正式美术未完成而阻塞玩法验证，但占位资产也必须满足最终尺寸、锚点和安全区契约。

...
状态：设计方向已确认；文本待审阅；Phase 4B 与 Phase 5 分拆计划的权威输入
...
若本文与旧视觉包或 Phase 5 计划中的单一静态场景、单一角色图片假设冲突，本文优先。本文不改变 Phase 1–4A 已确认的 Simulation、Session、Persistence、Digest 或 Story 所有权边界；Phase 4B 必须在冻结完整 ID catalog、GameView、Semantic actions、golden 和 Save fixtures 前纳入本文规定的表现选择与既有行为映射。
...
`ResolvedGameV1.sceneGraph` 保存冻结的 StageScene、variant、rig 和 HitMap descriptor catalog。GameView 只选择 catalog 中稳定的 ID；Application 再合并 resolved Presentation、内容偏好和临时 UI session state，产生不可变 `RuntimePresentationView`：
...
GameView + resolved Presentation
...
interface StageScenePresentationV1 {
  ...
  readonly layout: StrictJsonObject;
  readonly actors: readonly CharacterPlacementV1[];
  readonly interactionSurfaces: readonly InteractionSurfacePlacementV1[];
}
```

时间段、天气、季节、剧情、设施、Aura、地点状态或其他 Story Gameplay 条件可以由 Story 的 GameView projector 选择 variant；引擎不监听 Calendar，也不定义 morning/day/evening/night 枚举。换装预览、互动模式等临时 UI 选择只进入 application-owned UI session state，不反向进入 GameQueries。

Web-only Presentation provider 只解析已经投影的 ID、内容偏好和临时 UI 状态，不能再次读取 GameQueries，也不能建立第二套 Gameplay availability 计算。若选择会改变可用 Gameplay 行为或未来结果，相关原因必须先成为显式 Story State/Query；Presentation 只投影结果，不能把 Gameplay 规则藏在背景选择器中。
...
`direct` 必须恰好引用一个 enabled 默认行为；`choose` 必须至少包含两个有序行为；`open_surface` 必须引用存在且不会形成循环的 surface。缺失引用、循环、零行为或含多个默认行为在启动期以稳定 validation code 拒绝，运行时不猜测或自动改写模式。
...
Story 的 GameView projector 先根据 GameQueries 产生当前 `StageScene/variant` 选择，SemanticGamePort 从同一 GameQueries 产生 action catalog。Application 的 Interaction projector 只能消费该不可变 GameView、同一 Semantic action catalog、resolved Presentation、内容偏好和临时 UI session state，再投影有序行为描述：
...
## 12. Phase 4B 集成与 Phase 5 分拆

Phase 4B 必须先登记 StageScene/variant/rig/HitMap/behavior ID catalog，在 GameView 中投影当前 `StageScene/variant` 选择，并把 PoC InteractionBehavior 映射到已有 Semantic descriptors；Task 8 的 semantic integration 证明这些映射不形成第二套 gate。若不增加七日 PoC Gameplay，golden 和 Save fixtures 的命令序列保持原范围。

新增表现范围使现有 Phase 5 过大，应在本文审阅后拆成三个可独立验收的计划；同步更新 roadmap、文档索引并 supersede 旧的单份 Phase 5 plan。

### Phase 5A：UI Runtime Foundations

- AssetRegistry、PresentationReadPort 和 fallback；
- contribution registry、Runtime View bridge；
...
7. §7.3 的七条最小合法路径全部通过；缺失 surface、循环、零行为、`direct` 多默认和 `choose` 少于两个行为均以稳定 code 拒绝；
8. Pointer（鼠标/触摸/笔）、键盘、可访问行为列表、直接 Semantic invocation 和 Automation facade 对同一个有持久后果的行为复用同一 availability、preview、dispatch、rejection 与 Story Command 结果；
9. Presentation-only 行为可以由 Pointer、键盘和等价 DOM 入口到达，但不推进 commandSequence、不写 CommandLog；
10. 一次物理操作只激活一次；Overlay/VN/System 不穿透；Pointer cancel、窗口失焦和 StageScene 替换清理临时 interaction state；
11. 属性、关系、重复次数和 Narrative 后果只通过原子 Gameplay Command 修改，Phase 5 不新增任何此类合同；
12. CommandLog 原样记录 Story Command，且不自动包含坐标、DOM、设备、surface、target 或 renderer 私有数据；Replay 不重放 Semantic/HitMap/Presentation intent；
13. 内容成熟度 Schema 拒绝负数、非安全整数、重复 ID/值、未登记 required/maximum 和非法 default；
14. requiredLevel ≤ maximumLevel 的比较、Story default、Host 持久化、运行时切换、资源预加载过滤和标准 fallback 正确；
15. ContentPreference 不进入 State/Snapshot/Save/digest，不改变 Semantic revision、Module/State/Command 合同或 Gameplay availability；
16. 内容偏好降低不产生主线死路，不破坏 Save/Replay；所有 Gameplay invocation 保留 standard 等价入口；
17. DebugBundle.uiContext 有界且非权威，identity 不匹配时只作诊断摘要；
18. E2E Story 用中性目标和中性等级 fixture 证明机制，且不导入 PoC 内容；
19. 1024×768、768×1024、1600×1000、2560×1080、200% zoom 和 reduced-motion 下互动仍可操作；2560×1080 时 Stage 宽度不超过 1600、宽高比不超过 16:10，并保持居中留白；
20. 背景、角色和动画分别按 §5.3/§6.4 的登记顺序 fallback；不留下透明但仍响应 Pointer 的幽灵热点，失去兼容空间坐标时仍保留语义 DOM 入口；
21. fake Live2D renderer contract test 可以映射 pose/cue/target，而无需引入真实 SDK。
...
- StageSceneVariant 由 frozen catalog 和 Story/GameView 选择组成，不与时间绑定；
...
- Phase 4B 先冻结表现 ID、GameView 选择和既有 Semantic 映射，Phase 5 不新增 Gameplay；

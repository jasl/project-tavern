...
packages/base/src/contracts/
  ...
  application.ts                 # later SemanticGamePort contract
...
- Produces: StoryToolingEntryV1、StoryToolingSupportV1、ResolvedGameV1、StorySimulationFacetV1.createGameSimulation 和类型保持的 resolveGamePackageV1。
...
it("returns one deeply frozen complete ResolvedGame", () => {
  ...
  expect(result.resolved).toMatchObject({
    ...
    sceneGraph: { scenes: ["scene.synthetic.counter"] },
...
增加 type test，证明 resolver 返回值能够从具体 GamePackage 推断 gameSimulation、program、presentation 和 sceneGraph，而不需要 `specializeSandboxResolvedStoryV1` 这类 Story-owned 类型断言。再加两组 determinism case：模块级 named Rule/Resolver 在两次 define 中保持同一引用时通过；重新创建的函数引用、冲突的 provider ID 或不同 source digest 必须返回稳定 determinism failure。SceneGraph 本身只是 Strict JSON data，不参与 executable-reference 比较。
...
Expected: FAIL because ResolvedStoryV1 still drops uiSceneGraph, createProfile remains on the source facet, and StoryTooling names do not exist。
...
```bash
git add -- packages/base/src/contracts/game-package.ts packages/base/src/contracts/hotfix.ts packages/base/src/contracts/index.ts packages/base/src/authoring packages/base/src/index.ts packages/base/src/testkit packages/base/type-tests/phase1-consumer.test-d.ts packages/base/public-exports.v1.json apps/web/src/loader stories/sandbox
git diff --cached --check
...
- Produces: side-effect-free E2E StoryEntry、E2eSimulationProgramV1、E2ePresentationV1、E2eSceneGraphV1、rule/value patch slots and fixed Story tooling fixtures。
...
- scene.e2e.main：显示 counter、flow visible node 和正常操作；
- 一个阻塞 Narrative overlay：choice/rejoin；
- scene.e2e.summary：terminal summary。
...
- Produces: SemanticGamePortV1、SemanticActionDescriptorV1 envelope、createSemanticGamePortV1、E2eSemanticInvocationV1 and a no-DOM headless runner。
...
it("exposes only player-visible semantic state and available actions", () => {
  ...
  expect(port.observe()).toEqual({
    revision: 0,
    status: "idle",
    game: expect.any(Object),
    actions: expect.any(Array),
  });
  expect(port.observe()).not.toHaveProperty("snapshot");
  expect(port.observe()).not.toHaveProperty("hiddenConditions");
...
it("publishes status changes without inventing a gameplay revision", async () => {
  ...
  await fixture.publishBusyIdleWithoutReplacement();
...
it("routes a throwing semantic subscriber through the session failure hook", async () => {
  ...
  expect(fixture.observerFailures()).toHaveLength(1);
  expect(fixture.port.observe()).toMatchObject({ revision: 1, status: "idle" });
});
...
it("waits for a later stable revision without sleeping", async () => {
  ...
  await fixture.port.dispatch({ actionId: "action.e2e.increment", parameters: {} });
  await expect(awaited).resolves.toMatchObject({ revision: 1, status: "idle" });
});
```
...
```ts
export interface SemanticGamePortV1<TView, TActionDescriptor, TInvocation, TPreview, TResult> {
...
- 仅在 authoritative Snapshot 引用改变时递增 monotonic semantic revision，busy/idle publication 不重复递增；
- 每次 observe/actions 从 source 的当前 committed Gameplay state 重建 Queries，绝不缓存 Story state；
...
export type E2eSemanticInvocationV1 =
  | { readonly actionId: "action.e2e.increment"; readonly parameters: {} }
  | {
      readonly actionId: "action.e2e.choose";
      readonly parameters: { readonly choice: "left" | "right" };
    }
  | { readonly actionId: "action.e2e.continue"; readonly parameters: {} }
  | { readonly actionId: "action.e2e.complete"; readonly parameters: {} };
```

Action descriptor 至少包含 stable actionId、textId、enabled、按 authored order 排列的 reasons 和受控 options。preview 与 dispatch 都在实际调用时重新创建 Queries；dispatch 再由 executor 在 queue front 执行最终 guard。不存在 arbitrary command passthrough。
...
- waitForIdle revision behavior without sleep；
- no Snapshot、DebugTools or tooling reachability。
...
- [ ] Resolved SceneGraph is data-only `.ts`；Web renderer registry is `.tsx` in the application closure and is unreachable from direct Node Story/Headless imports。
...
- [ ] SemanticGamePort exposes only visible view/actions，provides standard subscribe，rebuilds preview Queries from a FIFO queue-front state-only read，and supports deterministic waitForIdle without sleep。

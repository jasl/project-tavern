...
- 在任何 Phase 2 编辑前运行只读 `pnpm verify:materialization`；缺失或 stale 必须以 `external_precondition.materialization_stale` 在改动前停止。`pnpm prepare:goal` 只属于路线图 R1.5，Phase 2 不得运行 registry-facing materialization writer。
...
- Modify: packages/base/src/contracts/presentation.ts
...
```bash
git status --short -- stories/sandbox stories/demo stories/e2e packages/modules
git ls-files --error-unmatch stories/sandbox/package.json stories/demo/package.json packages/modules/package.json
rm -rf -- \
  stories/sandbox/dist stories/sandbox/node_modules stories/sandbox/tsconfig.tsbuildinfo stories/sandbox/tsconfig.application.tsbuildinfo \
  stories/demo/dist stories/demo/node_modules stories/demo/tsconfig.tsbuildinfo \
  stories/e2e/dist stories/e2e/node_modules stories/e2e/tsconfig.tsbuildinfo \
  packages/modules/dist packages/modules/node_modules packages/modules/tsconfig.tsbuildinfo
test -z "$(git status --short --ignored stories/sandbox stories/demo stories/e2e packages/modules | awk '$1 == "!!" { print }')"
```
...
删除 packages/modules 三个 tracked files；从 E2E/PoC manifests、workspace policy、project references、license path prose 和 root scripts 中删除该包。确认已经采用“shared or Story-local Gameplay software”措辞的 `CONTRIBUTING.md` 保持 byte-identical，不再引用已删除的目录。packages/assets 保留。

在同一 atomic package migration 中冻结最终 Story importer boundary：

```json
{
  "stories/e2e dependencies": {
    "@project-tavern/base": "workspace:*",
    "@project-tavern/ui": "workspace:*",
    "@project-tavern/assets": "workspace:*",
    "@project-tavern/web": "workspace:*",
    "react": "19.2.7",
    "zod": "4.4.3"
  },
  "stories/poc dependencies": {
    "@project-tavern/base": "workspace:*",
    "@project-tavern/ui": "workspace:*",
    "@project-tavern/assets": "workspace:*",
    "@project-tavern/web": "workspace:*",
...
E2E preserves the migrated Sandbox UI/React importer semantics and adds no new external package。PoC adds the same React/UI and generic Web Loader/mount workspace boundary now, before its Scene application exists，so Phase 4/5 never need an undeclared cross-workspace import。Both Stories may import only public `@project-tavern/web` exports；禁止 `apps/web/src/**`、relative deep import 或 Story-specific reverse import。`react-dom`/`react-router-dom` remain Web-owned and are not Story dependencies。Lockfile changes must be limited to the `stories/e2e`/`stories/poc` workspace importer entries and removed `packages/modules`/old Story importers；R1.5 `materialization-lock.json` external package closure digest remains unchanged。
...
! rg -n "packages/modules|@project-tavern/modules" CONTRIBUTING.md
...
Expected: 所有命令退出 0；只有 E2E 和 PoC Story；两个 Story manifest 的 dependency map 与上述 exact object 完全相等；lockfile diff 只改变 workspace importers，不改变 R1.5 external closure；第二次 offline install 重建 workspace links，且不会访问 registry；checkpoint gate 不读取或重写尚待 Task 12 冻结的 fixture/golden。
...
同一 test 必须冻结 root `scripts/verify.mjs` 已将 `verify:balance` 精确替换为 `verify:determinism`，并断言 package.json 不再含 `verify:balance` 或 `verify:phase2:checkpoint`。这次 rename 的 tracked owners 是 `stories/e2e/scripts/verify-balance.mts`、`stories/e2e/package.json`、root `package.json`、`scripts/verify.mjs`、`scripts/verify.test.mjs`；它们必须全部进入本任务 staged set。
...
shasum -a 256 stories/e2e/fixtures/session-zero.json stories/e2e/golden/semantic-flow.json
...
```bash
git add -A -- stories/e2e/fixtures stories/e2e/golden stories/e2e/scripts stories/e2e/src/runtime/hotfix-integration.test.ts stories/e2e/src/runtime/dependency-failures.test.ts stories/e2e/package.json package.json scripts/verify.mjs scripts/verify.test.mjs scripts/verify-phase2-runtime.mts scripts/verify-phase2-runtime.test.mjs scripts/verify-phase2-checkpoint.mts scripts/verify-phase2-checkpoint.test.mjs scripts/run-script-tests.test.mjs
git diff --cached --check

...
- The PoC registers and ships only content level `standard = 0`. Its effective and default maximum level are both `0`; no `suggestive`, `sexual`, or `explicit` runtime text, behavior, media, asset slot, preload, test expectation, or hidden branch is added. The E2E Story uses neutral names for two ordered integer levels solely to verify the mechanism.
- A content-level descriptor is exactly `{ id: ContentMaturityLevelId, ordinal, nameTextId, descriptionTextId }`; a requirement stores only `requiredLevel`, and a preference stores only `maximumLevel`. Story catalogs own the `TextId` values and player-facing labels resolve only through `PresentationReadPortV1.text()`. No policy, requirement, preference, Host record, or renderer embeds a level name/description string.
...
- World-semantic icons are independent of the seven Stage renderer namespaces. Phase 5B registers the exact Story-owned `symbol.poc.*` IDs above through the Phase 5A `GameSymbolProviderV1`/`GameSymbolRegistryV1` surface; `@project-tavern/ui` owns no PoC ID, Lucide remains system-only, and symbol providers cannot become asset, Gameplay, or presentation-routing authorities.
...
- Create: `apps/web/src/preferences/content-preference-store.ts`
- Create: `apps/web/src/preferences/content-preference-store.test.ts`
- Create: `apps/web/src/preferences/index.ts`
- Modify: `apps/web/src/index.ts`
- Modify: `apps/web/type-tests/application-exports.test-d.ts`
- Test: `apps/web/src/preferences/content-preference-store.test.ts`

**Interfaces:**

- Consumes: Base `ContentMaturityPolicyV1`, `ContentPreferenceV1`, `ContentPreferencePortV1`, strict policy/ordinal parsers, `HostAtomicRecordStoreV1`, Story identity, and Web Host logger.
- Produces: `createWebContentPreferencePortV1`, one Story-scoped `settings/content-maturity.v1:<StoryId>` record, deterministic invalid-record fallback, CAS serialization, and the public `@project-tavern/web` preference export.

- [ ] **Step 1: Write the failing default, persistence, invalid-record, and isolation tests**

```ts
import { describe, expect, it, vi } from "vitest";
import { createMemoryHostRecordStoreV1 } from "@project-tavern/base";
import { createWebContentPreferencePortV1 } from "./content-preference-store.js";

const e2eBaseMaximumV1 = neutralTwoLevelPolicyV1.levels[0]!.ordinal;
const e2eExtraMaximumV1 = neutralTwoLevelPolicyV1.levels[1]!.ordinal;
...
describe("Web ContentPreferencePort", () => {
  it("uses the Story default without writing an absent record", async () => {
    ...
    const port = await createWebContentPreferencePortV1({
      records,
      storyId: "story.e2e",
      policy: neutralTwoLevelPolicyV1,
      reportWarning: vi.fn(),
    });
    expect(port.observe()).toEqual({ maximumLevel: 0 });
    expect(await records.list("settings")).toEqual([]);
  });

  it("persists and republishes one valid maximum level", async () => {
    const records = createMemoryHostRecordStoreV1();
    const first = await createE2ePreferencePortV1(records);
    const listener = vi.fn();
    first.subscribe(listener);
    await expect(first.set({ maximumLevel: e2eExtraMaximumV1 })).resolves.toEqual({
      kind: "updated",
      preference: { maximumLevel: 100 },
    });
    expect(first.observe()).toEqual({ maximumLevel: 100 });
    expect(listener).toHaveBeenCalledOnce();

    const second = await createE2ePreferencePortV1(records);
    expect(second.observe()).toEqual({ maximumLevel: 100 });
  });

  it("falls back after a removed policy ordinal and reports one bounded warning", async () => {
    const records = createPreferenceRecordFixtureV1({
      key: "content-maturity.v1:story.e2e",
      value: { contractRevision: 1, storyId: "story.e2e", maximumLevel: 100 },
    });
    const reportWarning = vi.fn();
    const port = await createWebContentPreferencePortV1({
      records,
      storyId: "story.e2e",
      policy: neutralBaseOnlyPolicyV1,
      reportWarning,
    });
    expect(port.observe()).toEqual({ maximumLevel: 0 });
    expect(reportWarning).toHaveBeenCalledWith({
      code: "content_preference.unregistered_level",
      storyId: "story.e2e",
      storedMaximumLevel: 100,
    });
  });

  it("rejects an unregistered set without opening Host storage", async () => {
    const fixture = await createBaseOnlyPreferenceFixtureV1();
    await expect(fixture.port.set({ maximumLevel: e2eExtraMaximumV1 })).resolves.toEqual({
      kind: "rejected",
      code: "content_maturity.unregistered_level",
    });
    expect(fixture.commits()).toBe(0);
    expect(fixture.port.observe()).toEqual({ maximumLevel: 0 });
  });
  ...
  it("isolates records by Story and never exposes a runtime capability", async () => {
    ...
    await e2e.set({ maximumLevel: e2eExtraMaximumV1 });
    expect(poc.observe()).toEqual({ maximumLevel: 0 });
  ...
  it("keeps a committed preference when a subscriber and warning reporter throw", async () => {
    ...
    await expect(fixture.port.set({ maximumLevel: e2eExtraMaximumV1 })).resolves.toMatchObject({
      kind: "updated",
    });
    expect(fixture.port.observe()).toEqual({ maximumLevel: 100 });
...
Run: `pnpm --filter @project-tavern/web exec vitest run src/preferences/content-preference-store.test.ts`
...
function parseContentPreferenceForPolicyResultV1(
  ...
  | { readonly kind: "unregistered_level" } {
  return policy.levels.some((level) => level.ordinal === preference.maximumLevel)
    ? Object.freeze({ kind: "parsed" as const, preference: Object.freeze({ ...preference }) })
    : Object.freeze({ kind: "unregistered_level" as const });
}
...
export async function createWebContentPreferencePortV1(
  ...
  let current = Object.freeze({ maximumLevel: decoded.maximumLevel });
  ...
  const persist = async (next: ContentPreferenceV1): Promise<ContentPreferenceSetResultV1> => {
    ...
    if (parsed.kind === "unregistered_level") {
      return Object.freeze({
        kind: "rejected" as const,
        code: "content_maturity.unregistered_level" as const,
      });
    }
    const bytes = canonicalJsonBytes({
      contractRevision: contentPreferenceRecordRevisionV1,
      storyId: input.storyId,
      maximumLevel: parsed.preference.maximumLevel,
    ...
    try {
      ...
      current = Object.freeze({ maximumLevel: parsed.preference.maximumLevel });
      notify();
      return Object.freeze({
...
`decodePreferenceOrDefaultV1` accepts only canonical JSON with exactly `contractRevision`, matching `storyId`, and a registered safe-integer `maximumLevel`. Absent records use `policy.defaultMaximumLevel` without a write. Invalid bytes, wrong Story identity, unsupported record revision, or an unregistered level each use the default and emit exactly one stable warning for that construction. Subscriber and diagnostic-reporter exceptions are isolated and cannot turn a successful Host commit into a failed public result. `set` returns `rejected/content_maturity.unregistered_level` before opening a Host transaction. Concurrent calls serialize through one local tail. A CAS conflict re-reads the latest revision and retries once; a second conflict or Host exception returns `failed/content_preference.storage_failed`, publishes nothing, and preserves the pre-operation preference snapshot.
...
port.set({ maximumLevel: input.policy.levels[0]!.ordinal });
...
expect(await collectProductionImportsV1("apps/web/src/preferences")).not.toMatch(
...
Run: `pnpm --filter @project-tavern/web exec vitest run src/preferences && pnpm verify:public-exports && pnpm verify:boundaries && pnpm typecheck && pnpm verify`

Expected: PASS; setting the E2E preference changes only its Host settings record, and the PoC remains at maximum level `0`.

- [ ] **Step 5: Commit the ordinary Host preference port**

```bash
git add -- apps/web/src/preferences apps/web/src/index.ts apps/web/type-tests/application-exports.test-d.ts
...
- Create: `packages/ui/src/runtime/runtime-presentation-store.ts`
- Create: `packages/ui/src/runtime/runtime-presentation-store.test.ts`
- Create: `packages/ui/src/runtime/use-runtime-presentation.ts`
- Modify: `packages/ui/src/runtime/index.ts`
- Modify: `packages/ui/src/index.ts`
- Create: `packages/ui/type-tests/runtime-presentation-public.test-d.ts`
- Test: `packages/ui/src/runtime/runtime-presentation-store.test.ts`
...
it("reprojects preference and UI state without inventing a Semantic revision", async () => {
  ...
  await fixture.preference.set({ maximumLevel: fixture.extraContentLevel.ordinal });
...
it("publishes the exact allowed asset list in first-use order", () => {
  ...
  expect(fixture.store.getSnapshot().requiredAssetIds).not.toContain("asset.e2e.background.extra");
});
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/runtime/runtime-presentation-store.test.ts`
...
it("keeps generic runtime and Story Web projection closures separated", async () => {
  expect(await collectProductionImportsV1("packages/ui/src/runtime")).not.toMatch(
    /stories\/|apps\/web/u,
  );
  expect(await collectNodeImportClosureV1("stories/e2e/src/story-entry.ts")).not.toMatch(
    /runtime-presentation|\.tsx|react/u,
  );
});
```

Run: `pnpm --filter @project-tavern/ui exec vitest run src/runtime src/assets && pnpm verify:boundaries && pnpm verify:cycles && pnpm typecheck && pnpm verify`
...
```bash
git add -- packages/ui/src/runtime packages/ui/src/index.ts packages/ui/type-tests/runtime-presentation-public.test-d.ts
git diff --cached --check
...
- Create: `packages/ui/src/stage/contracts.ts`
- Create: `packages/ui/src/stage/stage-scene-host.tsx`
- Create: `packages/ui/src/stage/stage-scene-host.module.css`
- Create: `packages/ui/src/stage/stage-scene-host.test.tsx`
- Create: `packages/ui/src/stage/index.ts`
- Modify: `packages/ui/src/index.ts`
- Create: `packages/ui/type-tests/stage-presentation-public.test-d.ts`
- Test: `packages/ui/src/stage/stage-scene-host.test.tsx`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/stage/stage-scene-host.test.tsx`
...
it.each([
  "stories/e2e/src/story-entry.ts",
  "stories/poc/src/index.ts",
  "stories/e2e/src/runtime/headless-runner.ts",
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/stage && pnpm verify:stories && pnpm verify:boundaries && pnpm verify:cycles && pnpm typecheck && pnpm verify`
...
```bash
git add -- packages/ui/src/stage packages/ui/src/index.ts packages/ui/type-tests/stage-presentation-public.test-d.ts
git diff --cached --check
...
- Create: `packages/ui/src/characters/contracts.ts`
- Create: `packages/ui/src/characters/character-renderer-registry.ts`
- Create: `packages/ui/src/characters/character-renderer-registry.test.ts`
- Create: `packages/ui/src/characters/character-host.tsx`
- Create: `packages/ui/src/characters/character-host.test.tsx`
- Create: `packages/ui/src/characters/static-character-renderer.tsx`
- Create: `packages/ui/src/characters/paper-doll-character-renderer.tsx`
- Create: `packages/ui/src/characters/character-renderers.test.tsx`
- Create: `packages/ui/src/characters/index.ts`
- Modify: `packages/ui/src/index.ts`
- Modify: `packages/ui/type-tests/stage-presentation-public.test-d.ts`
- Test: `packages/ui/src/characters/*.test.*`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/characters`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/characters && pnpm verify:assets && pnpm verify:boundaries && pnpm typecheck && pnpm verify`
...
```bash
git add -- packages/ui/src/characters packages/ui/src/index.ts packages/ui/type-tests/stage-presentation-public.test-d.ts
git diff --cached --check
...
- Modify: `packages/base/src/contracts/presentation.test.ts`
- Create: `packages/ui/src/interaction/contracts.ts`
- Create: `packages/ui/src/interaction/hit-test.ts`
- Create: `packages/ui/src/interaction/hit-test.test.ts`
- Create: `packages/ui/src/interaction/runtime-interaction-validation.ts`
- Create: `packages/ui/src/interaction/runtime-interaction-validation.test.ts`
- Create: `packages/ui/src/interaction/index.ts`
- Modify: `packages/ui/src/index.ts`
- Modify: `packages/ui/type-tests/stage-presentation-public.test-d.ts`
- Modify: `stories/e2e/src/story-contract.test.ts`
- Test: `packages/ui/src/interaction/hit-test.test.ts`
- Test: `packages/ui/src/interaction/runtime-interaction-validation.test.ts`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/interaction/hit-test.test.ts src/interaction/runtime-interaction-validation.test.ts`
...
`packages/ui/src/interaction/contracts.ts` imports the Base-owned types for internal use, and `packages/ui/src/interaction/index.ts` re-exports those exact symbols. A public type test proves Base and UI imports are mutually assignable; no second declaration is permitted.

```ts
import type { InteractionEntryModeV1, InteractionResolutionModeV1 } from "@project-tavern/base";
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/interaction && pnpm --filter @project-tavern/base exec vitest run src/contracts/presentation && pnpm --filter @project-tavern/story-e2e exec vitest run src/story-contract.test.ts && pnpm verify:stories && pnpm verify:boundaries && pnpm typecheck && pnpm verify`
...
```bash
git add -- packages/ui/src/interaction packages/ui/src/index.ts packages/ui/type-tests/stage-presentation-public.test-d.ts packages/base/src/contracts/presentation.test.ts stories/e2e/src/story-contract.test.ts
git diff --cached --check
...
- Create: `packages/ui/src/interaction/interaction-session-store.ts`
- Create: `packages/ui/src/interaction/interaction-session-store.test.ts`
- Create: `packages/ui/src/interaction/presentation-intent-router.ts`
- Create: `packages/ui/src/interaction/presentation-intent-router.test.ts`
- Create: `packages/ui/src/interaction/interaction-controller.ts`
- Create: `packages/ui/src/interaction/interaction-controller.test.ts`
- Create: `packages/ui/src/interaction/InteractionSurface.tsx`
- Create: `packages/ui/src/interaction/InteractionSurface.module.css`
- Create: `packages/ui/src/interaction/InteractionSurface.test.tsx`
- Create: `packages/ui/src/interaction/InteractionBehaviorList.tsx`
- Create: `packages/ui/src/interaction/InteractionBehaviorList.test.tsx`
- Modify: `packages/ui/src/interaction/contracts.ts`
- Modify: `packages/ui/src/interaction/index.ts`
- Modify: `packages/ui/src/index.ts`
- Test: remaining `packages/ui/src/interaction/*.test.*`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/interaction`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/interaction && pnpm --filter @project-tavern/web exec vitest run src/input && pnpm verify:semantic && pnpm verify:boundaries && pnpm typecheck && pnpm verify`
...
```bash
git add -- packages/ui/src/interaction packages/ui/src/index.ts
git diff --cached --check
...
- Create: `stories/e2e/src/presentation/interaction-fixture.ts`
- Create: `stories/e2e/src/presentation/runtime-presentation.ts`
- Create: `stories/e2e/src/presentation/runtime-presentation.test.ts`
- Create: `stories/e2e/src/presentation/ui-contributions.tsx`
- Create: `stories/e2e/src/presentation/ui-contributions.test.tsx`
- Create: `stories/e2e/src/presentation/e2e-settings-section.tsx`
- Create: `stories/e2e/src/presentation/e2e-settings-section.test.tsx`
...
- Test: affected `stories/e2e/src/presentation/*.test.*`

**Interfaces:**

- Consumes: the Phase 2 `E2eSceneGraphV1` with `stage_scene.e2e.main/summary`, `surface.e2e.counter`, the rect/circle/polygon HitMap, neutral `content.e2e.base/extra` policy, E2E `SemanticPublicationV1<E2eGameViewV1, E2eSemanticActionDescriptorV1, ...>`, Task 2 RuntimePresentation projector input/output, Tasks 3–6 Stage/Character/Interaction types, and the existing `action.e2e.increment` descriptor/invocation.
- Produces: additive data-only active variant/extra cue descriptors and their neutral TextCatalog entries in the existing `E2eSceneGraphV1`, exact inferred `E2eSemanticGamePortV1`/publication/action-descriptor aliases, `e2eInteractionFixtureV1` runtime fixtures, `E2ePresentationUiStateV1`, `E2eRuntimePresentationViewV1`, `E2eRuntimePresentationPublicationV1`, `projectE2eRuntimePresentationV1`, Web-only `e2eUiContributionsV1`, and `E2eSettingsSectionV1`; it does not replace the Phase 2 catalog, policy, Story entry, root, or Gameplay.

- [ ] **Step 1: Write the failing neutral catalog and startup-validation tests**

```ts
it("reuses the Phase 2 neutral two-level catalog with no PoC semantics", () => {
  expect(e2eContentMaturityPolicyV1.levels.map(({ id, ordinal }) => ({ id, ordinal }))).toEqual([
    { id: "content.e2e.base", ordinal: 0 },
    { id: "content.e2e.extra", ordinal: 100 },
  ]);
  expect(e2eContentMaturityPolicyV1.defaultMaximumLevel).toBe(0);
  const presentation = createE2ePresentationReadPortFixtureV1();
  for (const level of e2eContentMaturityPolicyV1.levels) {
    expect(presentation.text(level.nameTextId).text.trim()).not.toBe("");
    expect(presentation.text(level.descriptionTextId).text.trim()).not.toBe("");
  }
...
behavior.e2e.counter.extra_cue      # Phase 5B pure-Presentation behavior
cue.e2e.counter.extra
asset.e2e.background.base
asset.e2e.background.extra
asset.e2e.character.base
```

The Phase 2 HitMap already contains one rect, one circle, and one polygon area with overlapping-priority coverage; these shapes are mechanical fixtures, not anatomical regions. `behavior.e2e.counter.extra_cue` is an additional pure-Presentation behavior on the always-present counter figure target, so changing maximum level never adds/removes a HitMap target or creates a zero-behavior spatial target.
...
it("reuses the increment descriptor from the same publication", () => {
  ...
  const projected = projectE2eRuntimePresentationV1(
    e2eProjectionInputV1({
      semantic,
      maximumLevel: e2eContentMaturityPolicyV1.levels[0]!.ordinal,
    }),
  );
...
it("filters only extra presentation while preserving Gameplay availability", () => {
  ...
  const base = projectE2eRuntimePresentationV1(
    e2eProjectionInputV1({
      semantic,
      maximumLevel: e2eContentMaturityPolicyV1.levels[0]!.ordinal,
    }),
  );
  const extra = projectE2eRuntimePresentationV1(
    e2eProjectionInputV1({
      semantic,
      maximumLevel: e2eContentMaturityPolicyV1.levels[1]!.ordinal,
    }),
  );
  expect(findE2eBehaviorV1(base.view, "behavior.e2e.counter.extra_cue")).toBeUndefined();
  expect(findE2eBehaviorV1(extra.view, "behavior.e2e.counter.extra_cue")).toBeDefined();
  expect(collectE2eTargetIdsV1(base.view)).toEqual(collectE2eTargetIdsV1(extra.view));
  expect(findE2eBehaviorV1(base.view, "behavior.e2e.counter.increment")?.route).toMatchObject({
    descriptor: incrementDescriptor,
  });
  expect(base.requiredAssetIds).toEqual(["asset.e2e.background.base", "asset.e2e.character.base"]);
  expect(extra.requiredAssetIds).toContain("asset.e2e.background.extra");
  const extraIncrement = findE2eBehaviorV1(extra.view, "behavior.e2e.counter.increment");
  expect(extraIncrement?.route).toMatchObject({ descriptor: incrementDescriptor });
...
export function projectE2eRuntimePresentationV1(
  ...
  const extraCue = requireInteractionBehaviorV1(
    input.resolvedCatalog,
    "behavior.e2e.counter.extra_cue",
  );
  const extraAllowed = isContentRequirementAllowedV1(
    extraCue.requiredLevel,
    input.contentPreference.maximumLevel,
  );
  return buildFrozenE2eRuntimeProjectionV1({ input, increment, extraAllowed });
}
```

`interaction-fixture.ts` contains Strict JSON descriptors and parsed stable IDs only. `runtime-presentation.ts` lives in the Web application closure, imports no GameQueries/module/executor/State contract, and receives no Story runtime callback except its own module-level projector function. The active variant uses the already-published `game.flow.status`; it does not make the engine depend on Flow any more than the PoC projector makes it depend on Calendar.

- [ ] **Step 5: Implement the neutral Web contributions and verify closure/digest partition**

`e2eUiContributionsV1` registers one CSS background, one layered neutral counter character, the existing counter Interaction surface, one compact HUD, one neutral Overlay, and the existing Narrative/System hosts in the seven Phase 5A namespaces. `E2eSettingsSectionV1` is an application-only React component outside `e2eUiContributionsV1` and all seven renderer registries; the E2E application root constructs it with exactly `ContentPreferencePortV1` and `PresentationReadPortV1`, it renders the registered level names/descriptions, and calls only `contentPreference.set`. It contains no Semantic/Gameplay/Snapshot/capability access. The extra cue changes only a CSS label/visual token. Its cue/control/level labels are stable neutral `TextId` entries added to the existing E2E TextCatalog and resolved only through `PresentationReadPortV1`; runtime views retain those IDs. Add these closure/digest/runtime tests:
...
it("partitions neutral presentation and simulation identity", () => {
  const base = resolveE2ePresentationIdentityFixtureV1(baseFixtureCatalogV1);
  const changed = resolveE2ePresentationIdentityFixtureV1(changedFixtureCatalogV1);
...
it.each(e2eContentMaturityPolicyV1.levels)(
  "keeps standard Gameplay reachable at level $id",
  ({ ordinal }) => {
    renderE2ePresentationFixtureV1({ maximumLevel: ordinal });
...
```bash
git add -- stories/e2e/src/presentation stories/e2e/src/story-contract.test.ts stories/e2e/src/index.ts stories/e2e/tsconfig.application.json
git diff --cached --check
...
- Create: `stories/poc/src/presentation/runtime/contracts.ts`
- Create: `stories/poc/src/presentation/runtime/project-poc-runtime-presentation.ts`
- Create: `stories/poc/src/presentation/runtime/project-poc-runtime-presentation.test.ts`
- Create: `stories/poc/src/presentation/runtime/interaction-behaviors.ts`
- Create: `stories/poc/src/presentation/runtime/interaction-behaviors.test.ts`
- Modify: `stories/poc/src/test/story-validation.test.ts`
- Test: `stories/poc/src/presentation/runtime/*.test.ts`

**Interfaces:**

- Consumes: the Phase 4B frozen `pocResolvedPresentationCatalogV1` with its StageScene/variant/rig/HitMap/behavior graph and explicit heroine layer-to-asset selection, atomic `SemanticPublicationV1<PocGameViewV1, PocSemanticActionDescriptorV1, ...>`, `PocGameViewV1.hud.day`, `PocGameViewV1.hud.phase`, Phase 5B runtime types, the `standard = 0` PoC content policy, and application UI/Overlay/Interaction state.
...
- [ ] **Step 5: Prove standard-only filtering, exact assets, digest partition, and no Gameplay expansion**

Add these standard-only, asset, and identity tests:

```ts
it("ships exactly one standard PoC content level", () => {
  expect(pocContentMaturityPolicyV1.levels.map(({ id, ordinal }) => ({ id, ordinal }))).toEqual([
    { id: "content.poc.standard", ordinal: 0 },
  ]);
  expect(pocContentMaturityPolicyV1.defaultMaximumLevel).toBe(0);
  const standard = pocContentMaturityPolicyV1.levels[0]!;
  const presentation = createPocPresentationReadPortFixtureV1();
  expect(presentation.text(standard.nameTextId).text.trim()).not.toBe("");
  expect(presentation.text(standard.descriptionTextId).text.trim()).not.toBe("");
  expect(JSON.stringify(standard)).not.toMatch(/"name"\s*:|"description"\s*:/u);
});
...
```bash
git add -- stories/poc/src/presentation/runtime stories/poc/src/test/story-validation.test.ts
git diff --cached --check
...
- Create: `stories/poc/src/presentation/hud/PocHud.module.css`
- Create: `stories/poc/src/presentation/hud/PocHud.test.tsx`
- Create: `stories/poc/src/presentation/scenes/PocMainMenuScene.tsx`
- Create: `stories/poc/src/presentation/scenes/PocTavernScene.tsx`
- Create: `stories/poc/src/presentation/scenes/PocMarketScene.tsx`
- Create: `stories/poc/src/presentation/scenes/PocWorldMapScene.tsx`
- Create: `stories/poc/src/presentation/scenes/PocWeekSummaryScene.tsx`
- Create: `stories/poc/src/presentation/scenes/PocScenes.module.css`
- Create: `stories/poc/src/presentation/scenes/scenes.test.tsx`
...
- Create: `stories/poc/src/presentation/overlays/RelationshipOverlay.tsx`
...
- Create: `stories/poc/src/presentation/symbols/poc-game-symbols.tsx`
- Create: `stories/poc/src/presentation/symbols/poc-game-symbols.module.css`
- Create: `stories/poc/src/presentation/symbols/poc-game-symbols.test.tsx`
...
Each Overlay receives a narrow view slice, the exact action descriptor, and the Story semantic port. Policy, Purchase, TavernPlan, Facility, and WorldAction build only the strict invocation variants allowed by descriptor options, call `preview`, render the returned player-visible result/reasons, and then pass that exact invocation to `dispatch`. Inventory, Ledger, Relationship, and RunSummary are read-only projections. No component calculates AP cost, cash guard, demand, revenue, relationship availability, 2D6, ending, or command payload outside the typed option construction already authorized by its descriptor.
...
- Create: `apps/web/src/routing/hash-router.ts`
- Create: `apps/web/src/routing/hash-router.test.ts`
- Create: `apps/web/src/routing/index.ts`
- Modify: `apps/web/src/index.ts`
- Create: `stories/e2e/src/application/create-e2e-presentation-runtime.ts`
- Create: `stories/e2e/src/application/create-e2e-presentation-runtime.test.ts`
...
it("composes the E2E Web presentation over its existing single game runtime", async () => {
  ...
  for (const level of e2eContentMaturityPolicyV1.levels) {
    expect(fixture.runtime.presentationRead.text(level.nameTextId).text.trim()).not.toBe("");
    expect(fixture.runtime.presentationRead.text(level.descriptionTextId).text.trim()).not.toBe("");
  }
});
...
it("composes the PoC Web presentation without a second Gameplay view source", async () => {
  ...
  expect(fixture.runtime.contentPreference.observe()).toEqual({ maximumLevel: 0 });
...
it("keeps generic Web source free of Story imports", async () => {
  expect(await collectProductionImportsV1("apps/web/src")).not.toMatch(
...
Both roots mount the Phase 5A `SettingsLauncherV1` in the always-reachable System layer and route it to exactly one `SettingsDialogV1`; this is production composition, not a browser-test fixture. The E2E dialog renders `E2eSettingsSectionV1`, one neutral content-level select backed by `ContentPreferencePortV1` for mechanism testing. Each option value is the registered descriptor's decimal `ordinal`; its visible name and description come only from `presentationRead.text(level.nameTextId/descriptionTextId)`. Selection looks the ordinal up in the frozen policy before calling `set` and never casts arbitrary DOM text/number into a branded level. The PoC dialog supplies no maturity selector because only `standard` exists; it renders the Story-localized truthful empty-settings text through `PresentationReadPortV1`. Opening/closing either Settings dialog changes only System UI session state, leaves a blocking Narrative mounted underneath, and never changes Semantic revision or Gameplay. Neither root imports Story tooling, DevDock, capability session overrides, Automation Bridge, HMR adapter, or Phase 5C browser globals.
...
- Create: `apps/web/playwright.interaction.config.ts`
- Create: `apps/web/e2e/interaction/e2e-interaction.spec.ts`
- Create: `apps/web/e2e/interaction/poc-stage.spec.ts`
- Create: `apps/web/e2e/interaction/interaction-accessibility.spec.ts`
- Create: `apps/web/e2e/interaction/interaction-input.spec.ts`
- Modify: root `package.json`
...
- the E2E extra-level select changes only the extra cue/background and leaves the increment control's enabled state and visible Semantic revision unchanged;
...
- the PoC opens on the tavern Idle stage with the top-card HUD; opening Purchase shows `stage_scene.poc.market`; closing returns to tavern; the standard-only root exposes no adult/extra content setting;
...
test("changes only neutral extra presentation preference", async ({ page }) => {
  await page.goto(`${e2eWebUrl}/#/play`);
  const before = await visibleSemanticRevisionV1(page);
  const increment = page.getByRole("button", { name: "增加计数" });
  const enabled = await increment.isEnabled();
  await page.getByLabel("测试表现级别").selectOption("100");
  await expect(page.getByText("可选测试演出")).toBeVisible();
...
{
  "scripts": {
    "test:e2e:interaction": "playwright test --config apps/web/playwright.interaction.config.ts"
  }
}
```
...
```bash
git add -- scripts/ui/serve-story-roots.mts scripts/ui/serve-story-roots.test.ts apps/web/playwright.interaction.config.ts apps/web/e2e/interaction package.json
git diff --cached --check
...
- [ ] `ContentPreferencePortV1` persists as one Story-scoped Host settings record with strict policy validation, deterministic default/recovery, CAS behavior, and no Runtime Capability, Save, Snapshot, Gameplay, Semantic revision, Replay, or identity effect.
- [ ] The PoC registers and ships only `content.poc.standard = 0`. The neutral E2E two-level fixture proves integer ordering, preference persistence, exact asset filtering, runtime switching, and standard Gameplay parity without importing PoC/adult semantics.
...
- [ ] The PoC heroine has only `target.poc.heroine.figure`; no body-part target, daily touch reward, repeat counter, persistent outfit, content above standard, or new relationship rule enters the revision.
- [ ] Parameterized service-plan/purchase/WorldAction controls open a Story Overlay, retain the exact descriptor, construct only declared typed options, use Semantic preview, and dispatch the exact invocation. They never submit an empty/default command merely because a spatial target was activated.

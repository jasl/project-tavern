...
```text
packages/base/src/contracts/diagnostics.ts
packages/base/src/contracts/diagnostics.test.ts
...
- Modify: `packages/base/src/runtime/diagnostics/debug-bundle.ts`
- Modify: `packages/base/src/runtime/diagnostics/debug-bundle.test.ts`
- Create: `packages/ui/src/diagnostics/debug-ui-context.ts`
- Create: `packages/ui/src/diagnostics/debug-ui-context.test.ts`
- Modify: `packages/ui/src/diagnostics/index.ts`
- Create: `packages/ui/type-tests/diagnostics-public.test-d.ts`
- Modify: `packages/ui/package.json`
- Modify: `apps/web/src/application/create-game-runtime.ts`
- Modify: `apps/web/src/application/create-game-runtime.test.ts`
- Modify: `stories/poc/src/application/create-poc-presentation-runtime.ts`
- Modify: `stories/poc/src/application/create-poc-presentation-runtime.test.ts`
- Modify: `stories/e2e/src/application/create-e2e-presentation-runtime.ts`
- Modify: `stories/e2e/src/application/create-e2e-presentation-runtime.test.ts`
...
- Consumes: Phase 3 generic `DebugBundleEnvelopeV1<TUiContext>`, diagnostics-provider seam, provenance/digest checks and privacy scrubber; Phase 5B `RuntimePresentationPublicationV1`, `RuntimeStageSceneV1`, `RuntimeCharacterPresentationV1`, UI session projection, and `ContentPreferencePortV1`.
- Produces: concrete `DebugUiContextV1`, `DebugUiSessionSummaryV1`, `DebugPresentationRendererSummaryV1`, `DebugPresentationSummaryV1`, exact limits, strict `createDebugUiContextSchemaV1(policy)`, pure `createDebugUiContextV1`, and `classifyDebugUiContextUseV1`.

- [ ] **Step 1: Write the failing bounds, privacy, digest-independence, and identity tests**

```ts
import { describe, expect, it } from "vitest";
import { createDebugUiContextSchemaV1 } from "@project-tavern/base";
import {
  classifyDebugUiContextUseV1,
  createDebugUiContextV1,
} from "@project-tavern/ui/diagnostics";
...
describe("DebugBundle presentation context", () => {
  ...
  it("rejects a maturity value absent from the active Story policy", () => {
    const schema = createDebugUiContextSchemaV1(standardOnlyPolicyV1);
    expect(() =>
      schema.parse({
        ...validDebugUiContextV1,
        presentation: {
          ...validDebugUiContextV1.presentation,
          maximumContentLevel: 100,
        },
      }),
    ).toThrowError(/diagnostics\.ui_context_content_level_unknown/);
  });
...
```bash
pnpm --filter @project-tavern/base exec vitest run src/contracts/diagnostics.test.ts
pnpm --filter @project-tavern/ui exec vitest run src/diagnostics/debug-ui-context.test.ts
```
...
export interface DebugPresentationSummaryV1 {
  ...
  readonly maximumContentLevel: ContentMaturityLevelV1;
}
...
Use Phase 3's generic closed UI-context slot to define the concrete `DebugUiContextV1` above; the outer `DebugBundleEnvelopeV1.uiContext` remains optional, but a present V1 context has both required fields. `createDebugUiContextSchemaV1(policy)` closes decode over the active resolved Story policy. It rejects unknown keys; duplicate `characterId`; duplicate visible surface IDs; duplicate appearance-layer IDs within one character; any ID over 256 UTF-8 bytes; an over-limit renderer/appearance/surface/detail-stack array; coordinates; asset runtime paths; DOM values; callbacks or renderer instances; Live2D parameters; and maturity values absent from that policy. Different characters may legally reuse the same renderer, rig, pose, expression, or appearance-layer ID, so those cross-character repetitions are preserved.

`createDebugUiContextV1` is a pure structural projection over exactly `(1)` one cached `RuntimePresentationPublicationV1` whose view is constrained to the neutral `stage`, `characters`, and `interactionSurfaces` slices from 5B, `(2)` the current `ContentPreferencePortV1.observe()` value, and `(3)` a newly copied `DebugUiSessionSummaryV1`. It reads only stable IDs and booleans. It does not infer a runtime fallback result that is absent from the publication, inspect `layout`, action descriptors/invocations, HitMap geometry, renderer contributions/instances, asset providers, GameQueries, or Snapshot. The application reads the three sources once per bundle export; because the result is diagnostic-only, it records the consumed presentation revision instead of inventing cross-source atomicity.
...
Extend the existing diagnostics directory and UI-root export from Phase 5A, then add the one player-safe `@project-tavern/ui/diagnostics` package subpath in this task; do not create a competing diagnostics entry. The type test proves the new subpath accepts `RuntimePresentationPublicationV1` and neutral IDs but exports no Story type, Snapshot, GameSession, DebugTools, owner capability, DOM node, or renderer instance.
...
const gameRuntime = createGameRuntimeV1({
  ...gameInputs,
  uiContextSchema: createDebugUiContextSchemaV1(resolvedContentPolicy),
  readUiContext: () => {
...
```bash
pnpm --filter @project-tavern/base exec vitest run src/contracts/diagnostics.test.ts src/runtime/diagnostics
pnpm --filter @project-tavern/ui exec vitest run src/diagnostics
...
```bash
git add -- packages/base/src/contracts/diagnostics.ts packages/base/src/contracts/diagnostics.test.ts packages/base/src/contracts/index.ts packages/base/src/index.ts packages/base/public-exports.v1.json packages/base/src/runtime/diagnostics/debug-bundle.ts packages/base/src/runtime/diagnostics/debug-bundle.test.ts packages/ui/src/diagnostics packages/ui/type-tests/diagnostics-public.test-d.ts packages/ui/package.json apps/web/src/application/create-game-runtime.ts apps/web/src/application/create-game-runtime.test.ts stories/poc/src/application/create-poc-presentation-runtime.ts stories/poc/src/application/create-poc-presentation-runtime.test.ts stories/e2e/src/application/create-e2e-game-runtime.ts stories/e2e/src/application/create-e2e-game-runtime.test.ts stories/e2e/src/application/create-e2e-presentation-runtime.ts stories/e2e/src/application/create-e2e-presentation-runtime.test.ts
...
- Create: `packages/ui/src/debug/DebugLaunchers.tsx`
...
- Create: `packages/ui/src/debug/DevDockPortalCoordinator.tsx`
- Create: `packages/ui/src/debug/dev-dock.test.tsx`
- Create: `packages/ui/src/debug/index.ts`
- Create: `packages/ui/type-tests/debug-public.test-d.ts`
- Modify: `packages/ui/src/shell/game-shell.tsx`
- Modify: `packages/ui/src/shell/game-shell.module.css`
- Modify: `packages/ui/src/overlays/overlay-host.tsx`
- Modify: `packages/ui/src/overlays/overlay-host.test.tsx`
- Modify: `packages/ui/src/narrative/vn-layer.tsx`
- Modify: `packages/ui/src/narrative/vn-layer.test.tsx`
- Modify: `packages/ui/src/system/system-dialog-host.tsx`
- Modify: `packages/ui/src/system/system-dialog-host.test.tsx`
- Modify: `packages/ui/src/errors/runtime-failure-dialog.tsx`
- Modify: `packages/ui/src/errors/errors.test.tsx`
- Modify: `packages/ui/package.json`
- Modify: `stories/poc/src/application/poc-application-root.tsx`
...
Run: `pnpm --filter @project-tavern/ui exec vitest run src/debug/dev-dock.test.tsx`
...
- No file in `packages/ui/src/debug/**` imports `stories/**`, PoC/E2E IDs, Snapshot, GameSession, owner capabilities, or a debug command union.
- Expose only `@project-tavern/ui/debug`; the public type test proves Story panels are injected through `DevDockContributionSetV1` and no Story command/state type leaks from the subpath.
...
```bash
pnpm --filter @project-tavern/ui exec vitest run src/debug src/shell src/overlays src/narrative src/system src/errors
...
```bash
git add -- packages/ui/src/debug packages/ui/type-tests/debug-public.test-d.ts packages/ui/src/shell/game-shell.tsx packages/ui/src/shell/game-shell.module.css packages/ui/src/overlays/overlay-host.tsx packages/ui/src/overlays/overlay-host.test.tsx packages/ui/src/narrative/vn-layer.tsx packages/ui/src/narrative/vn-layer.test.tsx packages/ui/src/system/system-dialog-host.tsx packages/ui/src/system/system-dialog-host.test.tsx packages/ui/src/errors/runtime-failure-dialog.tsx packages/ui/src/errors/errors.test.tsx packages/ui/package.json stories/poc/src/application/poc-application-root.tsx stories/e2e/src/application/e2e-application-root.tsx
git diff --cached --check
...
- Create: `stories/poc/src/tooling-ui/index.ts`
- Create: `stories/poc/src/tooling-ui/ui-contributions.tsx`
- Create: `stories/poc/src/tooling-ui/ui-contributions.test.tsx`
...
- Modify: `stories/e2e/src/application/create-e2e-presentation-runtime.ts`
- Create: `stories/e2e/src/tooling-ui/index.ts`
- Create: `stories/e2e/src/tooling-ui/ui-contributions.tsx`
- Create: `stories/e2e/src/tooling-ui/ui-contributions.test.tsx`
...
```bash
pnpm --filter @project-tavern/web exec vitest run src/capabilities
...
```bash
git add -- apps/web/src/capabilities apps/web/src/index.ts stories/poc/src/tooling stories/poc/src/tooling-ui stories/poc/src/application stories/poc/package.json stories/e2e/src/tooling-ui stories/e2e/src/application stories/e2e/package.json
git diff --cached --check
...
- Produces: `BrowserAutomationOperationResultV1`, `BrowserAutomationBridgeV1`, and `installBrowserAutomationBridgeV1` at `globalThis.__PROJECT_TAVERN_AUTOMATION_V1__`.
...
it("installs and removes one exact non-enumerable global property", async () => {
  const fixture = createAutomationFixtureV1({ automationBridge: true });
  const installed = globalThis.__PROJECT_TAVERN_AUTOMATION_V1__!;
  expect(Object.getOwnPropertyDescriptor(globalThis, "__PROJECT_TAVERN_AUTOMATION_V1__")).toEqual({
    configurable: true,
    enumerable: false,
    value: installed,
    writable: false,
  ...
  expect(Object.hasOwn(globalThis, "__PROJECT_TAVERN_AUTOMATION_V1__")).toBe(false);
  fixture.dispose();
});
...
it("never revives a revoked facade when capability is re-enabled", async () => {
  const fixture = createAutomationFixtureV1({ automationBridge: true });
  const revoked = globalThis.__PROJECT_TAVERN_AUTOMATION_V1__!;
  await fixture.capabilities.setEnabled("automation_bridge", false);
  await fixture.capabilities.setEnabled("automation_bridge", true);
  const current = globalThis.__PROJECT_TAVERN_AUTOMATION_V1__!;
...
it("never overwrites a facade owned by another live installer", () => {
  ...
  const owned = globalThis.__PROJECT_TAVERN_AUTOMATION_V1__!;
  expect(() => createAutomationFixtureV1({ automationBridge: true })).toThrowError(
    "automation.bridge_already_installed",
  );
  expect(globalThis.__PROJECT_TAVERN_AUTOMATION_V1__).toBe(owned);
  owner.dispose();
});
...
Run: `pnpm --filter @project-tavern/web exec vitest run src/automation/browser-automation-bridge.test.ts`
...
```bash
pnpm --filter @project-tavern/web exec vitest run src/automation
...
```bash
git add -- apps/web/src/automation apps/web/src/index.ts stories/poc/src/application stories/e2e/src/application
git diff --cached --check
...
it("accepts exactly the two Story-owned application roots", async () => {
  ...
  expect(result.applications).toEqual([
    { id: "e2e-web", root: "stories/e2e/src/application/entry.tsx" },
    { id: "poc-web", root: "stories/poc/src/application/entry.tsx" },
  ]);
  expect(result.developerRoots).toEqual([]);
});

it.each([
  ["web imports a Story", graphWithEdge("apps/web", "stories/poc")],
  ["E2E imports PoC", graphWithEdge("stories/e2e", "stories/poc")],
  ["default Story closure imports TSX", graphWithDefaultStoryTsxEdge()],
  ["production imports AIGC", graphWithEdge("stories/poc", "art-source/aigc")],
  ["production imports references", graphWithEdge("apps/web", "references")],
...
Object.freeze({
  "e2e-web": {
    entry: "stories/e2e/src/application/entry.tsx",
    output: "dist/e2e",
    toolingSpecifiers: [
      "@project-tavern/story-e2e/tooling",
      "@project-tavern/story-e2e/tooling-ui",
  ...
  "poc-web": {
    entry: "stories/poc/src/application/entry.tsx",
    output: "dist/poc",
    toolingSpecifiers: [
      "@project-tavern/story-poc/tooling",
      "@project-tavern/story-poc/tooling-ui",
...
Set `reuseExistingServer: false`, one isolated browser context per test, no shared IndexedDB/localStorage, and a snapshot path template independent of the developer's absolute path. `test:e2e:ui` is exactly `playwright test --config apps/web/playwright.ui.config.ts`.
...
```bash
git add -- apps/web/e2e/ui-targets.ts apps/web/e2e/ui-infrastructure.spec.ts apps/web/e2e/runtime-capabilities.spec.ts apps/web/playwright.ui.config.ts scripts/ui/serve-story-roots.mts scripts/ui/serve-story-roots.test.ts package.json
git diff --cached --check
...
- Create: `apps/web/e2e/semantic-publication-parity.spec.ts`
- Create: `apps/web/e2e/poc-primary-flow.spec.ts`
- Create: `apps/web/e2e/e2e-primary-flow.spec.ts`
- Modify: `stories/poc/src/application/poc-application-root.tsx`
- Modify: `stories/e2e/src/application/e2e-application-root.tsx`
...
test("one publication supplies GameView, actions, DOM state, and Automation", async ({ page }) => {
  ...
  const result = await page.evaluate(() => globalThis.__PROJECT_TAVERN_AUTOMATION_V1__!.observe());
...
Split the PoC first ordinary action and the first E2E Semantic command into one bounded test each whose title includes `@smoke`; subsequent Save/reload/workflow coverage stays in the surrounding `@phase5c @primary-flow` suite without that tag. Together with Task 6's two smoke cases, `rg -n "@smoke" apps/web/e2e` must find exactly four executable test titles.
...
```bash
git add -- apps/web/e2e/automation-bridge.spec.ts apps/web/e2e/semantic-publication-parity.spec.ts apps/web/e2e/poc-primary-flow.spec.ts apps/web/e2e/e2e-primary-flow.spec.ts stories/poc/src/application/poc-application-root.tsx stories/e2e/src/application/e2e-application-root.tsx
git diff --cached --check
...
it("keeps Phase 5C inspect-only", () => {
  expect(verifyPhase5cCommandsV1).toEqual([
    ...
    [
      "pnpm",
      ["--filter", "@project-tavern/ui", "exec", "vitest", "run", "src/debug", "src/diagnostics"],
    ],
    [
      "pnpm",
      [
...
- `DebugBundle.uiContext` contains only bounded StageScene/variant/renderer/appearance/interaction/content-preference summaries, is privacy-safe and non-authoritative, changes no state digest/replay result, and is diagnostic-only under presentation/application identity mismatch.

...
- The predecessor clean commit must already export the possibly empty `approvedPocAssetPacksV1` and pass `packages/assets/src/approved-poc-pack.test.ts`; Phase 0 inspects but never creates, selects or changes that handoff.
...
apps/web/package.json                              # idb and fake-indexeddb
packages/ui/package.json                           # Lucide, Radix and Noto Sans SC
...
if ! git cat-file -e HEAD:packages/assets/src/approved-poc-pack.ts || ! git cat-file -e HEAD:packages/assets/src/approved-poc-pack.test.ts; then echo external_precondition.asset_handoff_missing >&2; exit 1; fi
if ! pnpm exec vitest run packages/assets/src/approved-poc-pack.test.ts; then echo external_precondition.asset_handoff_invalid >&2; exit 1; fi
pnpm verify
git rev-parse HEAD
```
...
```bash
pnpm exec vitest run packages/assets/src/approved-poc-pack.test.ts
pnpm verify
git diff --check
...
```bash
git add -- .node-version .gitignore package.json apps/web/package.json packages/ui/package.json pnpm-lock.yaml
...
| occupied `127.0.0.1:{4173,4174,41731,41732}`                               | `external_precondition.port_unavailable`               |
...
2. requires non-empty repository-local/effective `git config user.name` and `user.email`, then proves it can exclusively bind and close `127.0.0.1:4173`, `4174`, `41731`, and `41732` without starting a long-lived server;

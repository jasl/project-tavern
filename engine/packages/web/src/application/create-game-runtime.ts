...
} from "../capabilities/runtime-capability-session-overlay.js";
...
export async function createGameRuntimeV1<TApplication, TDisposition = unknown, TUiContext = never>(
  ...
  const persistedCapabilities = await createWebCapabilityPreferencesV1(input.host);
  const capabilitySession = createRuntimeCapabilitySessionOverlayV1(
    persistedCapabilities,
    input.sessionRequestedCapabilities ?? [],
  );
